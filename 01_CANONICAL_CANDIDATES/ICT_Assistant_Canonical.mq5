...
#property description "ICT/SMC/MMM آموزشی و تحلیلی برای XAUUSD — رسم + آموزش با hover فارسی. نویسنده: خالق صالحی"
...
void UpdateWyckoffPhase()
{
   ...
   string reason="ساختاری شناسایی نشده (PS/SC/BC در پنجره نیست)";
   if(g_wyckRange.hasClimax)
   {
      if(!g_wyckRange.hasAR)
      { ph=WP_A; reason="فاز A — کلایمکس ثبت شد؛ منتظر AR (رالی تعیین‌کنندهٔ سقف/کف ساختار)"; }
      else if(!g_wyckRange.hasST)
      { ph=WP_A; reason="فاز A — AR مرز ساختار را ساخت؛ منتظر ST (بازگشت با حجم سبک‌تر) برای پایان فاز A"; }
      else if(!g_wyckRange.hasSOS)
      {
         if(g_wyckRange.hasShakeout)
         { ph=WP_C; reason="فاز C — شیکاوت (Spring/UTAD) انجام شد؛ منتظر SOS/SOW برای ورود به فاز D"; }
         else
         { ph=WP_B; reason=StringFormat("فاز B — ساخت علت (Cause): ساختار %s بین %.5f و %.5f با آزمون‌های پیوسته",
                                       g_wyckRange.isAccum?"Accumulation":"Distribution",
                                       g_wyckRange.floor, g_wyckRange.ceiling); }
      }
      else
      {
         if(g_wyckRange.barsBeyond>=InpWyckoffPhaseE_Bars)
         { ph=WP_E; reason=StringFormat("فاز E — روند خارج از ساختار (%d کندل پیوسته بیرون؛ %s)",
                                       g_wyckRange.barsBeyond, g_wyckRange.isAccum?"Markup":"Markdown"); }
         else
         { ph=WP_D; reason=StringFormat("فاز D — %s صادر شد؛ روند درون ساختار، شمارش LPS/LPSY (%s)",
                                       g_wyckRange.isAccum?"SOS":"SOW", g_wyckRange.hasLPS?"دارد":"ندارد"); }
      }
   }
...
void DetectSupplyDemand(const double &o[], const double &h[], const double &l[], const double &c[], int shift, double atr)
{
   ...
   for(int baseBars=1; baseBars<=MathMax(1,InpSD_BaseMaxBars); baseBars++)
   {
      ...
      // candles, break of structure, volume, imbalance/FVG» معیارهای قدرت زون)
      {
         ...
         z.strength=strength;
         z.strengthText=StringFormat("خروج %.2f×ATR | پایه %.2f×ATR | %d کندل پایه | %s",
...
void UpdateRTM(const double &o[], const double &h[], const double &l[], const double &c[], int shift, double atr)
{
   ...
   if(compress && expand)
   {
      g_rtmNote=StringFormat("RTM: انبساط پس از فشر‌دگی %d کندلی (میانگین %.2f ATR) — جهت: %s",need,avg/atr,(dir>0?"بالا":"پایین"));
      PushRTM(RTM_EXPANSION, t, c[shift], h[shift], l[shift], dir,
              StringFormat("Expansion: دامنهٔ کندل %.2f ATR پس از فشر‌دگی %.2f ATR (میانگین %d کندل)", rng/atr, avg/atr, need));
   }
   else if(compress)
   {
      g_rtmNote=StringFormat("RTM: فشر‌دگی فعال (%d کندل، میانگین %.2f ATR) — منتظر انبساط",need,avg/atr);
      int n=ArraySize(g_rtm);
      if(n==0 || g_rtm[n-1].evt!=RTM_COMPRESSION)
         PushRTM(RTM_COMPRESSION, t, (boxHi+boxLo)/2.0, boxHi, boxLo, 0,
                 StringFormat("Compression: میانگین دامنهٔ %d کندل %.2f ATR (آستانه %.2f) — انرژی ذخیره شده", need, avg/atr, InpRTM_CompressRangeATR));
   }
   ...
   if(rng>atr*InpRTM_MomentumATR && (body/rng)>=InpRTM_MomentumBody && dir!=0)
   {
      ...
      if(closesNearExt)
         PushRTM(RTM_MOMENTUM, t, c[shift], h[shift], l[shift], dir,
                 StringFormat("Momentum: دامنه %.2f ATR و بدنه/دامنه %.0f%% با بستهٔ نزدیک اکستریم در جهت %s",
...
void UpdateProfile()
{
   ...
   if(CopyHigh(_Symbol,PERIOD_CURRENT,0,need,h)<=0) return;
   if(CopyLow(_Symbol,PERIOD_CURRENT,0,need,l)<=0) return;
   ...
   g_profileNote=StringFormat("Profile: POC %.2f | VA %.2f–%.2f | HVN %d · LVN %d (tick-volume — نه حجم واقعی؛ VA با قاعدهٔ دو-ردیفی CQG)",
...
string AMTDayTypeName(uint t)
{
   switch(t)
   {
      case 1: return "Trend Day (روز روندی)";
      case 2: return "Normal Variation Day (نوسان طبیعی)";
      case 3: return "Normal Day (روز معمول)";
      case 4: return "Neutral Day (خنثی — هر دو طرف IB شکسته)";
      case 5: return "Non-trend Day (بی‌روند — IB حفظ شده)";
...
string AMTOpenTypeName(uint t)
{
   switch(t)
   {
      case 1: return "Open-Drive (باز شدن بیرون رنج روز قبل)";
      case 2: return "Open-Test-Drive (تست IB و سپس حرکت)";
      case 3: return "Open-Reject-Reverse (رد IB و بازگشت)";
      case 4: return "Open-Auction (باز شدن درون ارزش)";
      default: return "نامعلوم";
   }
}
...
void UpdateProfileAMT(datetime barTime, double atr)
{
   ...
   g_profileDaily.balanceNote=(px<=refHi && px>=refLo)
      ? "BALANCE (پذیرش درون Value Area)"
      : "IMBALANCE (خروج از Value Area)";
...
void UpdateFamilies(const double &o[], const double &h[], const double &l[], const double &c[], int shift, double atr)
{
   ...
   if(shift==1)
   {
      ...
      string barKind=g_brooksInfo.isTrendBar? "Trend bar" : (g_brooksInfo.isDoji? "Doji/small"
                  : (g_brooksInfo.isSignalBar? (g_brooksInfo.bullSignal? "Bull signal bar":"Bear signal bar") : "عادی"));
      ...
      g_brooksNote=StringFormat("Brooks: %s | Always-In: %s (۲۰ EMA %.5f) | شمارش پولبک: %s",
...
bool EnsureSessionBars(ENUM_TIMEFRAMES tf, int need)
{
   ...
   if(copied<=0) return false;
   if(CopyHigh(_Symbol, tf, 0, copied, h)<=0) return false;
   if(CopyLow(_Symbol, tf, 0, copied, l)<=0) return false;
   if(CopyOpen(_Symbol, tf, 0, copied, o)<=0) return false;
   if(CopyClose(_Symbol, tf, 0, copied, cl)<=0) return false;
...
int OnInit()
{
   ...
   if(InpExplainOpen==EXPLAIN_OPEN_CLICK)
      Print("ICT Assistant (v0.1) | توضیح آموزشی: روی هر خط/باکس کلیک کن تا کامل توضیح بدهد. "
            "کلیک بعدی روی همان آبجکت یا روی فضای خالی، یا کلید ESC، پنل را می‌بندد");
   else
      Print("ICT Assistant (v0.1) | توضیح آموزشی: حالت HOVER فعال است — موس را روی خط/باکس ببر (ورودی InpExplainOpen)");
   ...
   {
      Print("خطا: ساخت ATR handle ناموفق بود");
      return INIT_FAILED;
   }
   ...
   if(g_brooksEmaHandle==INVALID_HANDLE)
      Print("هشدار: ساخت هندل ۲۰ EMA ناموفق بود؛ وضعیت Always-In خنثی می‌ماند");
               ...
               long midDispId, ENUM_FVG_KIND kind, ENUM_TIMEFRAMES tf, bool birthTouch)
{
   ...
   f.mitigated=false; f.inverted=false; f.invalidated=false;
   f.ceTouched=false;
...
// "Mitigated" به‌تنهایی هرگز کافی نیست، و "بعد از Sweep بودن" هم به‌تنهایی
...
string SBSyncLabel()
{
   ...
   if(sb=="") return "—";
   string sync=(g_mtfConflict? "تضاد MTF" : (g_htfBias==DIR_NONE? "بدون Bias" : "HTF-LTF همسو"));
   return sb+" | "+sync;
}
...
{
   string dirTxt=(o.direction==DIR_BULL)? " (تقاضا)" : " (عرضه)";
...
void UpdateReverseRisk(double curClose, double atrValue, datetime barTime, double curHigh, double curLow)
{
   ...
   for(int i=0;i<ArraySize(g_fvgs);i++)
   {
      ...
      if(d>=best) continue;
      best=d;
      g_rrLevel=RRFvgKindName(g_fvgs[i].kind)+(g_fvgs[i].direction==DIR_BULL? " (صعودی)":" (نزولی)");
   ...
   { score+=25; why+="+۲۵ نقدینگیِ سمت Bias جارو شد و کندل پشت سطح بست (SFP) — "; }
   ...
   { score+=15; why+="+۱۵ Exhaustion: قدرت ادامه ضعیف شده — "; }
   if(g_rrLegProg>=85.0)
   ...
   if(g_leg.valid)
   {
      ...
      { score+=10; why+="+۱۰ Bias صعودی ولی قیمت در Premium لگ (ورود دیرهنگام) — "; }
      if(g_htfBias==DIR_BEAR && curClose<g_leg.eq)
      { score+=10; why+="+۱۰ Bias نزولی ولی قیمت در Discount لگ (ورود دیرهنگام) — "; }
   }
   if(g_hasDOL && g_rrDolDist<=0.5)
   { score+=15; why+=StringFormat("+۱۵ هدف نقدینگی تقریباً پر شد (فاصله %.2f ATR) — ", g_rrDolDist); }
   if(!g_mtfConflict)
   { score-=10; why+="−۱۰ همهٔ تایم‌فریم‌ها هم‌جهت Bias هستند (ادامه محتمل‌تر) — "; }
   if(score<0) score=0;
   ...
   // --- ۶) ثبت شاهد در CSV (منبع اندازه‌گیری درصد واقعی) ---
   if(!InpWriteReverseRisk) return;
   int h=DiagOpen("ICT_Assistant_Canonical_ReverseRisk_"+_Symbol+".csv");
      ...
      if(WindowForDayBack(0,0,0,0,0,barTime,dayStart,dayEnd,dH,dL,dCnt,true) && dayStart>0)
      {
         ...
         if(CollectWindowRange(PERIOD_CURRENT, dayStart, g_asiaEnd, hi, lo, dCnt) && hi>0)
         {
            if(hi>g_asiaHigh)
            {
               ...
               g_amdPriceNote=StringFormat("M تأیید: فتیلهٔ روز تا %.2f بیرون سقف آسیا (%.2f) — اگر برگشت و Displacement نزولی، Distribution نزولی = شورت", hi, g_asiaHigh);
            ...
            {
               g_amdPriceStage=AMD_MANIPULATION;
               g_amdPriceNote=StringFormat("M تأیید: فتیلهٔ روز تا %.2f بیرون کف آسیا (%.2f) — اگر برگشت و Displacement صعودی، Distribution صعودی = لانگ", lo, g_asiaLow);
            }
            else g_amdPriceNote="A فعال: قیمت داخل رنج آسیا — سوئپ (M) هنوز رخ نداده؛ هیچ الزامی نیست";
         }
      }
   }
...
void UpdateDOL(double curClose, double atrValue)
{
   ...
   if(bestId==-1)
   {
      g_dolRejectReason = (rejectedRoom>0)
         ? StringFormat("همهٔ %d سطح هم‌جهت نزدیک‌تر از حداقل %.2f ATR بودند؛ R:R واقعی ساخته نمی‌شد", rejectedRoom, InpDOL_MinRoomATR)
         : "سطح نقدینگی هم‌جهت با Bias و دست‌نخورده (FRESH) وجود ندارد";
      return;
   }
   ...
   g_currentDOL.breakdown = StringFormat("External %d + HTF %d + نوع %d + ساختار %d + نزدیکی %d = %d",
                                         wScope,wHTF,wType,wStruct,wProx,g_currentDOL.score);
   g_currentDOL.narrative = StringFormat("DOL از %s انتخاب شد | فاصله %.2f ATR | %d سطح بررسی شد | تفکیک: %s",
...
void UpdateSetupLifecycle(datetime barTime, double closePrice, double highPrice, double lowPrice)
{
   ...
   if(slHit)
   {
      ...
      g_setupInvalidCount++;
      g_setupLifeReason=StringFormat("کندل %s با close %s از SL %s رد شد (جهت ستاپ %s) → ستاپ باطل شد",
                                     TimeToString(barTime,TIME_DATE|TIME_MINUTES),
   ...
   else if(tp1Hit)
   {
      ...
      g_setupTP1Count++;
      g_setupLifeReason=StringFormat("هدف اول (۱R = %s) در %s لمس شد → پیگیری این ستاپ بسته شد",
...
void ArmSetupLifecycle(datetime barTime)
{
   ...
   g_setupArmedCount++;
   g_setupLifeReason=StringFormat("ستاپ READY در %s مسلح شد | entry %s | SL %s | TP1 %s | جهت %s",
                                  TimeToString(barTime,TIME_DATE|TIME_MINUTES),
...
void UpdateSetup(double curClose, double atrValue, datetime barTime)
{
   ...
   // داشت؛ حالا خودِ چرخه هم باید اثبات‌شده و تازه باشد.
   {
      ...
      for(int i=ArraySize(g_events)-1;i>=0;i--)
      {
         ...
         g_setup.chainText   =StringFormat("%s #%s در %s | Sweep #%s | Displacement #%s",
   ...
   for(int pass=0;pass<2 && fvgId==-1;pass++)
      for(int i=ArraySize(g_fvgs)-1;i>=0;i--)
      {
         ...
         if(g_fvgs[i].direction!=g_htfBias) continue;
   ...
   if(hasM5 && m5Mid>=oteLow && m5Mid<=oteHigh)
   {
      ...
      g_setup.zoneSource="FVG روی تایم‌فریم SETUP (M5), "+TimeToString(m5Time,TIME_DATE|TIME_MINUTES);
      zoneSelected=true;
   }
   ...
   g_setup.slSource=StringFormat("ATR %.2f×%.2f=%.2f | کف پوینتی %d pts=%.2f | StopsLevel %d pts=%.2f → بافر %.2f",
   ...
   int q=0; string qb="";
   if(chainIsMss){ q++; qb+="۱ MSS تأییدشده؛ "; }
   if(fvgId!=-1){ q++; qb+="۲ FVG causal؛ "; }
   if(obId!=-1){ q++; qb+="۳ OB معتبر؛ "; }
   ...
   if(obKind==OBK_CORE || obKind==OBK_EXTREME){ q++; qb+="۴ OB از نوع Core/Extreme (نه Standalone)؛ "; }
   if(obExtreme){ q++; qb+="۵ OB روی اکسترمم لگ؛ "; }
   if(hasM5){ q++; qb+="۶ ناحیه از تایم‌فریم SETUP؛ "; }
   POIObj bestPoi;
   if(FindBestPOI(g_htfBias, bestPoi) &&
      (bestPoi.kind==POIK_BREAKER || bestPoi.kind==POIK_OB || bestPoi.kind==POIK_MITIGATION))
   { q++; qb+="۷ بهترین POI ناحیهٔ ساختاری است؛ "; }
   if(curClose>=oteLow && curClose<=oteHigh){ q++; qb+="۸ قیمت داخل باند OTE؛ "; }
   if(g_trend.phase==PHASE_INITIATION || g_trend.phase==PHASE_EXPANSION){ q++; qb+="۹ فاز روند ادامه‌دار؛ "; }
   if(rr>=InpMinRR+0.5){ q++; qb+="۱۰ R:R با حاشیهٔ راحت"; }
   ...
   {
      g_setup.status="WAITING_ENTRY_MODEL ("+g_modelReason+")";
      return;
   }
   if(q<InpMinQualityScore)
   {
      g_setup.status=StringFormat("WAITING_QUALITY_LOW (%d از ۱۰)", q);
      return;
   }
...
void UpdateRangeLiquidity(double atrValue)
{
   ...
   if(height>maxH)
   {
      g_rangeReject=StringFormat("ارتفاع %.2f از حد مجاز %.2f (%.1f ATR) بیشتر است — روند است، نه رنج", height, maxH, InpRangeMaxATR);
      return;
   }
...
void UpdateHtfInternalStructure(datetime barTime)
{
   ...
   if(g_htfInternalDir==DIR_NONE)
      g_htfInternalReason=StringFormat("در پنجرهٔ %d کندلی، HH/HL یا LH/LL تأییدشده نیست", window);
}
...
void UpdateTrendPhase(datetime asOf)
{
   ...
   if(g_htfBias==DIR_NONE)
   {
      g_trend.phaseReason="Bias مالک H4 هنوز تأیید نشده است";
      return;
   }
   ...
   {
      g_trend.phaseReason="رویداد HTF تعیین‌کنندهٔ Bias در رجیستری پیدا نشد";
      return;
   }
   ...
   if(g_reversal.confirmed && g_reversal.dir==g_htfBias)
   {
      ...
      g_trend.phaseReason="برگشت تأییدشده سطح محافظت‌شدهٔ خارجی در جهت Bias فعلی";
   }
   else if(g_trend.ageBars<=InpTrendYoungBars)
   {
      g_trend.phase=PHASE_INITIATION;
      g_trend.phaseReason=StringFormat("سن %d کندل HTF کمتر یا مساوی %d است", g_trend.ageBars, InpTrendYoungBars);
   }
           ...
           g_exhaustion.state==EXH_RANGE_TRANSITION)
   {
      ...
      g_trend.phaseReason="Exhaustion ضعف را گزارش می‌کند: "+ExhaustionStateToStr(g_exhaustion.state);
   }
   else
   {
      g_trend.phase=PHASE_EXPANSION;
      g_trend.phaseReason=StringFormat("سن %d کندل HTF و هیچ هشدار ضعفی ثبت نشده است", g_trend.ageBars);
   }
}
...
void UpdatePOIRegistry(datetime asOf, double atrValue)
{
   ...
   {
      if(!g_fvgs[i].causal || g_fvgs[i].invalidated) continue;
      AddPOI(g_fvgs[i].id, POIK_FVG, g_fvgs[i].direction, g_fvgs[i].top, g_fvgs[i].bottom,
   ...
   for(int i=ArraySize(g_obs)-1;i>=0;i--)
   {
      ...
      AddPOI(g_obs[i].id, k, g_obs[i].direction, g_obs[i].top, g_obs[i].bottom,
      ...
      if(g_obs[i].direction==g_htfBias && g_obs[i].tf!=PERIOD_CURRENT)
      {
         for(int j=0;j<ArraySize(g_fvgs);j++)
         {
            if(!g_fvgs[j].causal || g_fvgs[j].invalidated || g_fvgs[j].inverted) continue;
            if(g_fvgs[j].direction!=g_obs[i].direction) continue;
                                 ...
                                 long fvgId, long obId, bool zoneInOte, bool legAligned)
{
   ...
   if(m2022){ g_modelReason="چرخهٔ MSS با Sweep و Displacement زنجیرشده و FVG هم‌جهت موجود است"; return MODEL_ICT2022; }
   if(mBfo) { g_modelReason="BOS/CHoCH با Displacement و هم‌پوشانی FVG + OB هم‌جهت موجود است"; return MODEL_BOS_FVG_OB; }
   if(mSweep){ g_modelReason="Sweep نقدینگی رخ داده و ناحیهٔ ورود داخل باند OTE همان لگ است"; return MODEL_SWEEP_ENTRY; }
   if(mOte) { g_modelReason="لگ معتبر هم‌جهت Bias است و ناحیهٔ ورود داخل باند OTE قرار دارد"; return MODEL_OTE_ONLY; }
...
void ScoreSmartMoneyReversal(ENUM_DIRECTION dir, datetime barOpen, datetime barCloseTime)
{
   ...
   {
      g_reversal.smrScore++;
      g_reversal.smrReason+=StringFormat("Sweep نقدینگی %s #%s؛ ", wantHighSide?"سمت خرید (BSL)":"سمت فروش (SSL)", IdToStr(sweepId));
   }
   ...
   long zoneId=-1;
   for(int i=ArraySize(g_fvgs)-1;i>=0;i--)
      if(g_fvgs[i].direction==dir && g_fvgs[i].causal && !g_fvgs[i].invalidated){ zoneId=g_fvgs[i].id; break; }
   if(zoneId==-1)
      for(int i=ArraySize(g_obs)-1;i>=0;i--)
   ...
   if(StringLen(g_reversal.smrReason)==0)
      g_reversal.smrReason="هیچ شاهد اضافی برای Smart Money Reversal پیدا نشد؛ ";
...
void UpdateReversalEngine(datetime asOf, double closePrice, double atrValue)
{
   ...
   if(!InpEnableReversalGate)
   {
      ...
      g_reversal.reason="دروازهٔ برگشت با ورودی InpEnableReversalGate خاموش است؛ هیچ برگشتی تأیید نمی‌شود";
      return;
   }
   ...
   if(guardBias==DIR_NONE)
   {
      ...
      g_reversal.reason="تا وقتی Bias مالک H4 تأیید نشود، هیچ برگشتی اعلام نمی‌شود";
      return;
   }
   if(!g_reversal.snapGuardOk)
   {
      g_reversal.state="WAITING_PROTECTED_LEVEL";
      g_reversal.reason="سویینگ محافظت‌شدهٔ خارجی در رجیستری H4 پیدا نشد (آخرین رویداد H4 سویینگ محافظ نداده یا با سقف InpMaxSwings حذف شده است)";
      return;
   }
   if(g_reversal.snapBarTime<=0 || g_reversal.snapBarClose<=0.0)
   {
      g_reversal.state="WAITING_HTF_DATA";
      g_reversal.reason="دادهٔ بستهٔ کافی روی HTF برای سنجش سطح محافظت‌شده وجود ندارد";
      return;
   }
   ...
   if(g_reversal.snapGuardLevelTime>0 && g_reversal.snapBarTime<=g_reversal.snapGuardLevelTime)
   {
      ...
      g_reversal.reason="سطح محافظت‌شده تازه ثبت شده است؛ سنجش از کندل بستهٔ بعدی HTF انجام می‌شود";
      return;
   }
   ...
   if(!broke)
   {
      if(!g_reversal.confirmed)
      {
         ...
         g_reversal.reason=StringFormat("سطح محافظت‌شدهٔ خارجی (%s %.2f) با close %.2f آخرین کندل بستهٔ HTF رد نشده است؛ برگشتی تأیید نمی‌شود",
   ...
   g_reversal.reason=StringFormat("کندل بستهٔ %s در %s با close %.2f فراتر از سطح محافظت‌شدهٔ خارجی (%s %.2f) بسته شد؛ این تنها مسیر اعلام برگشت تأییدشده است — فتیله کافی نیست",
                                  EnumToString(InpHTF), TimeToString(g_reversal.confirmedTime,TIME_DATE|TIME_MINUTES),
...
void PersistReversalDiagnostics(datetime barTime)
{
   ...
   int handle=DiagOpen("ICT_Assistant_Canonical_Reversal_Diag.csv");
   if(handle==INVALID_HANDLE) return;
   ...
   if(!needHeader)
   {
      ...
      if(!headerOk)
      {
         ...
         DiagDrop("ICT_Assistant_Canonical_Reversal_Diag.csv");
         FileClose(handle);
         FileDelete("ICT_Assistant_Canonical_Reversal_Diag.csv",FILE_COMMON);
         handle=DiagOpen("ICT_Assistant_Canonical_Reversal_Diag.csv");
         if(handle==INVALID_HANDLE) return;
         needHeader=true;
      }
   }
   if(needHeader)
      FileWrite(handle,"ChartBar","Symbol","State","BiasSnapshot","GuardId","GuardPrice","GuardSide","HtfBar","HtfClose","Broke","ConfirmedTime","ConfirmedClose","BarsSince","SmartMoneyReversal","SMRScore","SMRMax","SweepId","DispId","ZoneId","EventId","BuildStamp");
...
{
   color clr = f.direction==DIR_BULL?InpColorBull:InpColorBear;
   string stateTxt = "FRESH imbalance";
   if(!f.causal)        { clr=clrGray;          stateTxt="imbalance without causal displacement"; }
   if(f.mitigated)      { clr=clrGoldenrod;     stateTxt="touched (mitigated) - not fresh anymore"; }
   if(f.inverted)       { clr=clrMediumOrchid;  stateTxt="iFVG: closed through, polarity inverted"; }
   if(f.invalidated)    { clr=clrDimGray;       stateTxt="expired/invalid"; }
...
{
   color clr = o.direction==DIR_BULL?clrDodgerBlue:clrMagenta;
   ...
   if(o.state==OB_MITIGATED) { clr=clrGoldenrod;   stateTxt="MITIGATED (touched)"; }
   if(o.state==OB_BROKEN)    { clr=clrSlateGray;   stateTxt="BROKEN (structural violation)"; }
   if(o.state==OB_BREAKER)   { clr=clrDeepSkyBlue; stateTxt="BREAKER (retested after polarity flip)"; }
   if(o.state==OB_INVALID)   { clr=clrDimGray;     stateTxt="INVALID (no displacement/structure link)"; }
...
void DrawRejection(const RejectionObj &r)
{
   ...
   if(r.rejectionState==REJECTION_TOUCHED) clr=clrGoldenrod;
   if(r.rejectionState==REJECTION_INVALID) clr=clrDimGray;
...
void DrawLocationLevels()
{
   ...
   DrawLocationLine("EQ",eq,clrGold,"");
   DrawLocationLine("OTE_LOW",oteLow,clrAqua,"");
   DrawLocationLine("OTE_HIGH",oteHigh,clrAqua,"");
   DrawLocationLine("GOLDEN",golden,clrMagenta,"");
   ...
   DrawTextObj(pdName, g_leg.endTime, g_leg.endPrice, pdTxt,
               ...
               " | Mitigation/بازگشت به EQ یا OTE = فرصت ورود هم‌جهت؛ فاز Premium/Discount لگ را از EQ بخوان");
}
...
void DrawDOLLine()
{
   ...
   ObjectSetInteger(0,dolName,OBJPROP_COLOR,clrOrangeRed);
...
void FreezeLayerObject(const string nm)
{
   ...
   ObjectSetString(0,nm,OBJPROP_TOOLTIP,
      "FROZEN HISTORY | از پنجرهٔ نمایش فعلی بیرون افتاده (سقف InpMaxDrawnZones / InpMaxDrawnLevels) "
      +"و به‌عنوان شاهد تاریخی نگه داشته شده است، نه حذف. موس را روی آن نگه دار تا توضیح کامل فارسی بیاید."
      +(StringLen(origStyle)>0?" | original line style: "+origStyle:""));
...
void DrawLiquidityLayer()
{
   ...
   for(int i=ArraySize(g_liquidity)-1; i>=0 && drawn<InpMaxDrawnLevels; i--)
   {
      ...
      bool highSide = IsHighSideLiquidity(g_liquidity[i].type);
      color clr = swept ? clrGray : (highSide?clrTomato:clrMediumSeaGreen);
      if(g_liquidity[i].isHTF && !swept) clr = highSide?clrOrangeRed:clrLime;
   ...
   if(!InpHideInvalidatedObjects)
   {
      ...
      for(int i=ArraySize(g_liquidity)-1; i>=0 && drawnInvalid<invalidCap; i--)
      {
         ...
         string tip = StringFormat("%s | %s | INVALID: price accepted beyond this level (close-through) | price=%s",
                       ...
                       g_liquidity[i].scope==SCOPE_EXTERNAL?"External (HTF)":"Internal (LTF)",
                       DoubleToString(g_liquidity[i].price,_Digits));
         DrawHLineObj(name, g_liquidity[i].price, clrDimGray, STYLE_DOT, 1, tip);
         drawnInvalid++;
      }
   }
}
...
void DrawSweepLayer()
{
   ...
   for(int i=ArraySize(g_liquidity)-1; i>=0 && drawn<InpMaxDrawnLevels; i--)
   {
      ...
      bool highSide = IsHighSideLiquidity(g_liquidity[i].type);
      color clr = highSide?clrOrangeRed:clrLime;
...
void DrawSessionLayer()
{
   ...
   for(int back=0; back<InpKillzoneDaysBack; back++)
   {
      ...
      if(InpDrawAsianRange && WindowForDayBack(back,InpAsiaStartHourNY,0,InpAsiaEndHourNY,0,ref,s,e,hi,lo,cnt,true))
         DrawBoxObj("ICTv13_SESS_ASIA_"+sfx, s, hi, e, lo, clrSlateBlue, false, true, STYLE_DOT, 1,
      ...
      if(InpDrawKillzoneBoxes)
      {
         if(WindowForDayBack(back,InpLondonStartHourNY,0,InpLondonEndHourNY,0,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_LON_"+sfx, s, hi, e, lo, clrDodgerBlue, false, true, STYLE_DOT, 1,
         ...
         if(WindowForDayBack(back,InpNY_KZ_StartHourNY,0,InpNY_KZ_EndHourNY,0,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_NYAM_"+sfx, s, hi, e, lo, clrOrangeRed, false, true, STYLE_DOT, 1,
         ...
         if(WindowForDayBack(back,InpLondonCloseStartHourNY,0,InpLondonCloseEndHourNY,0,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_LONCL_"+sfx, s, hi, e, lo, clrGoldenrod, false, true, STYLE_DOT, 1,
         ...
         if(WindowForDayBack(back,InpNYPM_StartHourNY,InpNYPM_StartMinuteNY,InpNYPM_EndHourNY,InpNYPM_EndMinuteNY,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_NYPM_"+sfx, s, hi, e, lo, clrOrange, false, true, STYLE_DOT, 1,
      ...
      if(InpDrawSilverBullet)
      {
         if(WindowForDayBack(back,InpSB1_StartHourNY,0,InpSB1_EndHourNY,0,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_SB1_"+sfx, s, hi, e, lo, clrMagenta, false, true, STYLE_DASHDOT, 1,
         ...
         if(WindowForDayBack(back,InpSB2_StartHourNY,0,InpSB2_EndHourNY,0,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_SB2_"+sfx, s, hi, e, lo, clrMagenta, false, true, STYLE_DASHDOT, 1,
         ...
         if(WindowForDayBack(back,InpSB3_StartHourNY,0,InpSB3_EndHourNY,0,ref,s,e,hi,lo,cnt,true))
            DrawBoxObj("ICTv13_SESS_SB3_"+sfx, s, hi, e, lo, clrMagenta, false, true, STYLE_DASHDOT, 1,
...
void DrawSetupLayer()
{
   ...
   if(obId!=-1 || fvgId!=-1)
      DrawBoxObj("ICTv13_SETUP_ZONE", t1, zTop, t1 + PeriodSeconds()*InpZoneExtendBars, zBot,
                 g_setup.active?clrGold:clrDimGray, false, true, STYLE_SOLID, 2,
                 StringFormat("Entry zone = %s | status=%s | refines with CE/OTE",
                              obId!=-1?"Order Block":"FVG", g_setup.status));

   if(g_setup.active)
   {
      DrawHLineObj("ICTv13_SETUP_ENTRY", g_setup.entry, clrGold, STYLE_SOLID, 1, "Setup Entry | "+g_setup.status);
...
void DrawReversalLevels()
{
   ...
   if(g_reversal.armed && g_reversal.levelPrice>0.0)
   {
      ...
      DrawHLineObj(name, g_reversal.levelPrice, g_reversal.confirmed?clrOrangeRed:clrOrange,
                   g_reversal.confirmed?STYLE_SOLID:STYLE_DASH, 2, tip);
      if(InpDrawLevelLabels)
         DrawTextObj(name+"_T", g_lastContextBarTime, g_reversal.levelPrice, "REVERSAL GATE", clrOrange, 8, tip);
   }
...
void DrawFamiliesLayer()
{
   ...
   if(InpEnableSupplyDemand && InpDrawSDZones)
   {
      ...
      for(int i=ArraySize(g_sd)-1; i>=0 && drawn<sdCap; i--)
      {
         ...
         color sc=isSupp?InpColorBear:InpColorBull;
         if(g_sd[i].state==SDS_FLIPPED) sc=clrGoldenrod;
         else if(g_sd[i].state==SDS_TESTED) sc=clrGray;
   ...
   {
      DrawHLineObj("ICTv13_PROF_POC", g_profileDaily.poc, clrGold, STYLE_SOLID, 1, "POC (tick-volume) | "+g_profileNote);
      DrawHLineObj("ICTv13_PROF_VAH", g_profileDaily.vah, clrAqua, STYLE_DOT, 1, "VAH | "+g_profileNote);
      DrawHLineObj("ICTv13_PROF_VAL", g_profileDaily.val, clrAqua, STYLE_DOT, 1, "VAL | "+g_profileNote);
      if(g_profileDaily.tpoValid)
      {
         DrawHLineObj("ICTv13_PROF_TPOC", g_profileDaily.tpoPoc, clrKhaki, STYLE_DASH, 1,
                      StringFormat("TPO POC (زمان‌محور — نه حجم) | VA %.2f–%.2f", g_profileDaily.tpoVah, g_profileDaily.tpoVal));
         DrawHLineObj("ICTv13_PROF_TVAH", g_profileDaily.tpoVah, clrKhaki, STYLE_DOT, 1, "TPO VAH (Value Area زمان‌محور)");
         DrawHLineObj("ICTv13_PROF_TVAL", g_profileDaily.tpoVal, clrKhaki, STYLE_DOT, 1, "TPO VAL (Value Area زمان‌محور)");
      }
      if(g_profileDaily.amtValid && g_profileDaily.ibHi>g_profileDaily.ibLo)
      {
         DrawHLineObj("ICTv13_PROF_IBH", g_profileDaily.ibHi, clrMagenta, STYLE_DOT, 1,
                      StringFormat("Initial Balance High (اولین %d دقیقهٔ روز)", InpProfileIB_Minutes));
         DrawHLineObj("ICTv13_PROF_IBL", g_profileDaily.ibLo, clrMagenta, STYLE_DOT, 1,
                      StringFormat("Initial Balance Low (اولین %d دقیقهٔ روز) — شکست آن جهت روز را می‌سازد", InpProfileIB_Minutes));
      }
      if(g_profileDaily.hasNakedPoc)
         DrawHLineObj("ICTv13_PROF_NPOC", g_profileDaily.nakedPoc, clrTomato, STYLE_DASHDOT, 2,
      ...
      if(InpDrawAMTOnChart)
      {
         for(int h=0; h<g_profileDaily.hvnCount; h++)
            DrawHLineObj("ICTv13_PROF_HVN"+IntegerToString(h), g_profileDaily.hvn[h], clrLime, STYLE_DOT, 1,
                         "HVN (High Volume Node) — گرهٔ پرحجم: قیمت این‌جا وقت/حجم زیاد گذرانده (آهنربا و مانع)");
         for(int l2=0; l2<g_profileDaily.lvnCount; l2++)
            DrawHLineObj("ICTv13_PROF_LVN"+IntegerToString(l2), g_profileDaily.lvn[l2], clrOrange, STYLE_DOT, 1,
...
void ExplainWyckoff(const WyckoffObj &w)
{
   ...
   switch(w.evt)
   {
      case WE_SC: name="SC (Selling Climax — اوج فروش)"; break;
      case WE_BC: name="BC (Buying Climax — اوج خرید)"; break;
      case WE_AR: name="AR (Automatic Rally — رالی خودکار)"; break;
      case WE_ST: name="ST (Secondary Test — آزمون دوم)"; break;
      case WE_SPRING: name="Spring (شکار استاپ کف رنج)"; break;
      case WE_UPTHRUST: name="Upthrust (شکار استاپ سقف رنج)"; break;
      case WE_SOS: name="SOS (Sign of Strength)"; break;
      case WE_SOW: name="SOW (Sign of Weakness)"; break;
      case WE_LPS: name="LPS (Last Point of Support)"; break;
      case WE_LPSY: name="LPSY (Last Point of Supply)"; break;
      case WE_TEST: name="Test (آزمون کم‌دامنهٔ مرز)"; break;
      default: name="Wyckoff";
   }
   g_expTitle="WYCKOFF: "+name;
   ExpAddWrapped(StringFormat("چیست: رویداد Wyckoff روی کندل %s در قیمت %s — رنج تحلیل: سقف/کف پنجرهٔ %d کندلی",
                 TimeToString(w.time,TIME_DATE|TIME_MINUTES), PriceS(w.price), InpWyckoffLookbackBars), clrWhite);
   ExpAddWrapped("چرا اهمیت دارد: Composite Man (پول بزرگ) با SC/BC اوج هیجان را می‌سازد و با Spring/Upthrust استاپ‌های رنج را می‌شکند؛ برگشت درون رنج با Test کم‌دامنه تأیید می‌شود", clrAqua);
   ExpAddWrapped(StringFormat("فاز فعلی تحلیل: %s — معنی: %s", g_wyckPhaseReason, g_wyckPhaseReason), clrLime);
   ExpAddWrapped("فیک/باطل: اگر بستهٔ کندل بیرون مرز رنج بماند (نه فتیله)، دیگر Spring نیست؛ شکست واقعی است و سناریوی رنج باطل می‌شود", clrOrange);
   ExpAddWrapped("کنترل خودت: فتیلهٔ کندل را با کف/سقف رنج مقایسه کن؛ عمق باید بیشتر از تنظیم InpWyckoffSpringATR×ATR باشد", clrSilver);
}
...
void ExplainSD(const SDObj &z)
{
   ...
   string patn="";
   if(z.kind==SDK_RBR) patn="Rally → Base → Rally";
   else if(z.kind==SDK_RBD) patn="Rally → Base → Drop";
   else if(z.kind==SDK_DBD) patn="Drop → Base → Drop";
   else if(z.kind==SDK_DBR) patn="Drop → Base → Rally";
   else if(z.kind==SDK_SUPPLY) patn="ناحیهٔ عرضهٔ ساده";
   else patn="ناحیهٔ تقاضای ساده";
   g_expTitle="S/D: "+kn+(z.roleSupply? " (Supply — عرضه)" : " (Demand — تقاضا)");
   ExpAddWrapped(StringFormat("چیست: الگوی %s = %s؛ بازهٔ ناحیه %s تا %s؛ پایان پایه: %s",
                 kn, patn, PriceS(z.bottom), PriceS(z.top), TimeToString(z.time,TIME_DATE|TIME_MINUTES)), clrWhite);
   ExpAddWrapped("تفکیک ادامه‌دهنده از برگشتی: RBR و DBD ادامهٔ روند پیشین‌اند (قیمت از همان سمت پایه بیرون می‌رود)؛ RBD و DBR برگشتی‌اند (پایه در سقف یا کف می‌شود و خروج مخالف آن است)", clrAqua);
   ExpAddWrapped(StringFormat("چرا معتبر است: خروج از پایه شارپ بوده (%.2f ATR، حداقل %.2f) — جذب سفارش نهادی را نشان می‌دهد", z.exitMove, InpSD_StrengthATR), clrAqua);
   ExpAddWrapped(StringFormat("نقش فعلی ناحیه: %s%s — %s",
                 z.roleSupply?"Supply (عرضه)":"Demand (تقاضا)",
                 z.flipped? StringFormat(" | این ناحیه FLIP شده در %s روی قیمت %s", TimeToString(z.flipTime,TIME_DATE|TIME_MINUTES), PriceS(z.flipLevel)) : "",
                 z.flipped? "یعنی نقشش عوض شده و حالا در جهت جدید معامله می‌شود" : "هنوز نقش عوض نشده"), clrLime);
   ExpAddWrapped(StringFormat("تازگی: %s (%d بار برخورد) — Fresh یعنی بدون برخورد؛ Tested یعنی استاپ‌های آن سمت مصرف شده و اعتبار کمتر است",
                 z.state==SDS_FRESH?"FRESH":z.state==SDS_TESTED?"TESTED":z.state==SDS_FLIPPED?"FLIPPED":"BROKEN", z.tests), clrLime);
   // فاز ۳۶: Strength of Zone (امتیاز ترکیبی مستند)
   ExpAddWrapped(StringFormat("قدرت ناحیه (Strength of Zone): %d از ۵ — تفکیک: %s | معیارهای مستند: خروج شارپ‌تر = قوی‌تر؛ پایهٔ تنگ‌تر = سفارش متراکم‌تر؛ ۳ تا ۵ کندل پایه = استاندارد منابع؛ هم‌زمانی با FVG = ضریب تقویت (trendspider: large candles/BOS/imbalance)",
                 z.strength, z.strengthText),
                 z.strength>=4?clrLime:(z.strength==3?clrAqua:clrOrange));
   if(z.tests>=2)
      ExpAddWrapped("هشدار Absorption: این ناحیه چند بار برخورد خورده و هر بار پذیرفته نشده؛ در زبان نهادی یعنی سفارش مخالف در حال جذب است — نواحی Tested ریسک بالاتر و ضریب موفقیت کمتری دارند (منبع: S&D — Fresh vs Tested)", clrOrange);
   ExpAddWrapped("فیک/باطل: بستهٔ قاطع از سمت مقابل نقش، ناحیه را FLIP می‌کند (Supply ↔ Demand) و از آن لحظه ناحیه با نقش جدید رهگیری می‌شود؛ اگر قیمت دوباره از سمت مقابل نقش جدید ببندد، ناحیه BROKEN و بی‌اعتبار است — ورود برعکس روی ناحیهٔ FLIPPED ریسک بالا دارد", clrOrange);
   ExpAddWrapped(StringFormat("کنترل خودت: پایه را پیدا کن (۱ تا %d کندل بستهٔ کم‌دامنه با سقف‌دامنه کمتر از %.2f ATR) و نسبت خروج به ATR را خودت اندازه بگیر",
...
void ExplainBrooks()
{
   ...
   ExpAddWrapped(StringFormat("چیست: خوانش کندل‌به‌کندل — نوع کندل: %s | Trend bar اگر بدنه/دامنه ≥ %.2f",
                 g_brooksInfo.isTrendBar? "Trend bar" : (g_brooksInfo.isSignalBar? "Signal bar" : (g_brooksInfo.isDoji? "Doji/small body":"عادی")),
                 InpBrooks_TrendBarRatio), clrWhite);
   ExpAddWrapped(StringFormat("وضعیت Always-In: %s — این یک «وضعیت» است، نه شمارش کندل همرنگ: قیمت نسبت به ۲۰ EMA (%.5f) و شیب %d کندلی همان EMA سنجیده می‌شود. تا وقتی Always-In صعودی است، هر پولبک فرصت خرید در جهت است، نه دلیل فروش برعکس",
                 g_brooksInfo.alwaysIn>0? "LONG (صعودی)" : (g_brooksInfo.alwaysIn<0? "SHORT (نزولی)":"خنثی (بی‌روند)"),
                 g_brooksInfo.ema20, InpBrooks_AI_EMASlopeBars), clrLime);
   ExpAddWrapped(StringFormat("شمارش پولبک: H%d / L%d — تعریف: H1 اولین کندلی است که **سقف کندل قبلی را رد می‌کند** در بافت روند صعودی؛ اگر آن تلاش شکست بخورد و قیمت پایین‌تر برود، تلاش بعدی H2 است و همین‌طور H3/H4 (گوه). پس H2 «کف بالاتر» نیست، «شمارش تلاش» است. پنجرهٔ شمارش: %d کندل",
                 g_brooksInfo.hAttempts, g_brooksInfo.lAttempts, InpBrooks_PullbackWindow), clrAqua);
   ExpAddWrapped(StringFormat("وضعیت کندل جاری: Entry bar=%s (کندل روند بلافاصله پس از Signal bar) | Signal bar=%s (فقط با بافت: بسته‌شدن نزدیک اکستریم در جهت و پس از لگ مخالف) | Failed breakout=%s",
   ...
   ExpAddWrapped(g_brooksInfo.inTradingRange
      ? StringFormat("Trading Range دوطرفه: فعال — مرزها %.2f تا %.2f (%d کندل) | وسط رنج %.2f مغناطیس است: تا وقتی قیمت وسط رنج است، انتظار نوسان می‌رود، نه روند؛ ورود در میانهٔ رنج بدترین مکان است (فاصله تا هر دو مرز کم)",
                     g_brooksInfo.trLow, g_brooksInfo.trHigh, g_brooksInfo.trBars, g_brooksInfo.trMid)
      : "Trading Range دوطرفه: فعلاً تشخیص داده نشد (بازار یک‌سویه یا روند) — شرط: در پنجرهٔ ۲۰ کندلی هم گاوها و هم خرس‌ها حضور مؤثر داشته باشند و قیمت به هر دو مرز نزدیک شده باشد",
   ...
   if(g_brooksInfo.trFailedBreakUp || g_brooksInfo.trFailedBreakDn)
      ExpAddWrapped(StringFormat("شکست ناموفق (Failed Breakout): %s — فتیله از مرز بیرون رفت ولی کندل داخل رنج بست؛ کلاسیک Brooks: شکست ناموفق اغلب حرکت مخالف می‌سازد (همان وارونگی الگو)",
                    g_brooksInfo.trFailedBreakUp?"سقف رنج":"کف رنج"), clrOrange);
   ExpAddWrapped(g_brooksInfo.mmValid
      ? StringFormat("Measured Move (AB=CD): A=%.2f → B=%.2f → C=%.2f → هدف D=%.2f (۱۰۰%% امتداد لگ اول) — Brooks: لگ دوم که اندازهٔ لگ اول را تکرار کند هدف محاسبه‌پذیر می‌دهد؛ در روند قوی هدف ۱۰۰۴، در روند ضعیف اغلب کمتر",
                     g_brooksInfo.mmA, g_brooksInfo.mmB, g_brooksInfo.mmC, g_brooksInfo.mmD)
      : "Measured Move (AB=CD): فعلاً الگوی معتبری یافت نشد — نیاز به سه پیوت تأییدشده با پولبک بین دو لگ",
      g_brooksInfo.mmValid?clrLime:clrSilver);
   ExpAddWrapped(g_brooksInfo.chSlope!=0
      ? StringFormat("کانال: Always-In %s و قیمت %s نسبت به ۲۰ EMA (%.5f) — خط کانال موازی خط روند، هدف/مغناطیس بعدی حرکت است؛ کانال تند اغلب با شکست به سمت مخالف پایان می‌یابد (Brooks فصل ۱۵)",
                     g_brooksInfo.chSlope>0?"صعودی":"نزولی", g_brooksInfo.chSlope>0?"بالای":"زیر", g_brooksInfo.ema20)
      : "کانال: شیب قابل‌توجهی تشخیص داده نشد (Always-In خنثی یا قیمت چسبیده به ۲۰ EMA)",
      g_brooksInfo.chSlope!=0?clrAqua:clrSilver);
   ExpAddWrapped("فیک/باطل: اگر شکست فقط با فتیله باشد و کندل داخل ببندد = Failed breakout و اغلب حرکت مخالف می‌سازد؛ و اگر Always-In خنثی شد (قیمت دو طرف ۲۰ EMA سرگردان باشد) شمارش H/L اعتباری ندارد", clrOrange);
   ExpAddWrapped(StringFormat("کنترل خودت: بدنه/دامنه را با %.2f و نسبت فتیله را با %.2f مقایسه کن؛ برای Doji آستانهٔ بدنه %.2f است",
...
{
   g_expTitle="RTM: "+RTMEventName(e.evt);
   ExpAddWrapped(StringFormat("چیست: %s — زمان کندل %s · قیمت محوری %s",
                 RTMEventName(e.evt), TimeToString(e.time,TIME_DATE|TIME_MINUTES), PriceS(e.price)), clrWhite);
   ...
   if(e.evt==RTM_TRAP)
      ExpAddWrapped("چه چی درسته / چی فیکه: درست = قیمت بعد از تله در جهت مخالف فتیله ادامه دهد و از انتهای مخالف بسته شود؛ فیک = برگشت فوری به داخل و بسته‌شدن آن‌طرف جعبه با کندل مخالف", clrLime);
   else if(e.evt==RTM_EXPANSION)
      ExpAddWrapped("چه چی درسته / چی فیکه: درست = ادامهٔ حرکت در همان جهت و بیرون‌ماندن قیمت از جعبه؛ فیک = برگشت سریع به داخل جعبه (انبساط کاذب)", clrLime);
   else if(e.evt==RTM_MOMENTUM)
      ExpAddWrapped("چه چی درسته / چی فیکه: درست = کندل بعد در جهت ممنتوم ببندد (follow-through)؛ فیک = کندلی که کل دامنهٔ ممنتوم را پس بدهد", clrLime);
   else if(e.evt==RTM_ENGULF)
      ExpAddWrapped("چه چی درسته / چی فیکه: پوشش بدنه باید در انتهای یک حرکت مخالف رخ دهد؛ اگر وسط رنج باشد صرفاً نوسان است", clrLime);
   else if(e.evt==RTM_REJECTION)
      ExpAddWrapped("چه چی درسته / چی فیکه: رد واقعی وقتی است که قیمت به قیمت‌های سطح قبلی‌تر برنگردد؛ اگر چند کندل بعد سطح دوباره آزمون و بازدید شود، رد ناتمام است", clrLime);
   else
      ExpAddWrapped("چه چی درسته / چی فیکه: فشر‌دگی فقط «کاهش دامنه» است؛ تا Expansion جهت‌دار نیاید، معاملهٔ فشر‌دگی بی‌ جهت است", clrLime);
   ExpAddWrapped(StringFormat("کنترل خودت: آستانه‌ها — فشر‌دگی %.2f×ATR روی %d کندل · ممنتوم %.2f×ATR و بدنه %.0f%% · فتیلهٔ رد %.0f%%",
...
void ExplainRTM()
{
   g_expTitle="RTM — فشر‌دگی، انبساط، تله، ممنتوم";
   ExpAddWrapped(StringFormat("چیست: RTM (Read The Market) چرخهٔ «فشر‌دگی → انبساط» و رویدادهای رفتاری داخل آن است. فشر‌دگی = میانگین دامنهٔ %d کندل کمتر از %.2f ATR (انرژی ذخیره‌شده)",
                 InpRTM_CompressBars, InpRTM_CompressRangeATR), clrWhite);
   ExpAddWrapped(StringFormat("وضعیت فعلی: %s", (g_rtmNote==""? "هیچ فشر‌دگی فعالی نیست":g_rtmNote)), clrAqua);
   ExpAddWrapped(StringFormat("رویدادهای رهگیری‌شده در رجیستری (%d مورد): Compress/Expand · Trap (فتیله بیرون جعبه و بسته داخل = شکار استاپ) · Momentum (دامنه ≥ %.2f ATR و بدنه ≥ %.0f%%) · Engulfing (پوشش بدنهٔ قبلی) · Rejection (فتیله ≥ %.0f%% دامنه و بسته در ثلث مخالف)",
                 ArraySize(g_rtm), InpRTM_MomentumATR, InpRTM_MomentumBody*100.0, InpRTM_RejectTailRatio*100.0), clrLime);
   if(ArraySize(g_rtm)>0)
   {
      RTMObj last=g_rtm[ArraySize(g_rtm)-1];
      ExpAddWrapped(StringFormat("آخرین رویداد: %s در %s — %s", RTMEventName(last.evt),
                    TimeToString(last.time,TIME_DATE|TIME_MINUTES), last.note), clrWhite);
   }
   ExpAddWrapped("نحوهٔ استفاده: ورود پس از Expansion در جهت آن، یا پس از Trap در جهت مخالف فتیله؛ Engulfing و Rejection تأییدکننده‌اند، نه به‌تنهایی دلیل ورود — و اگر قیمت از انتهای مخالف جعبهٔ فشر‌دگی بسته شود، سناریو باطل است", clrLime);
   ExpAddWrapped("فیک/باطل: انبساط بدون جهت مشخص یا کندلی که فتیلهٔ تله را بزند ولی بیرون همان جعبه ببندد، تلهٔ واقعی نیست؛ فشر‌دگی هم بدون Expansion نویز است", clrOrange);
   ExpAddWrapped(StringFormat("کنترل خودت: میانگین دامنهٔ %d کندل را با %.2f×ATR، و بدنه/دامنه را با %.2f مقایسه کن",
...
void ExplainProfile()
{
   ...
   ExpAddWrapped(StringFormat("چیست: توزیع حجم روی ردیف‌های قیمتی از %d کندل اخیر — POC %.2f | VAH %.2f | VAL %.2f",
                 InpProfileLookbackBars, g_profileDaily.poc, g_profileDaily.vah, g_profileDaily.val), clrWhite);
   // ---------- فاز ۳۴: Auction Market Theory ----------
   if(g_profileDaily.tpoValid)
      ExpAddWrapped(StringFormat("TPO (Time Price Opportunity): POC زمان‌محور %.2f | VA زمانی %.2f–%.2f — این عدد از «شمارش زمان در هر ردیف» می‌آید، نه حجم؛ پس با POC حجمی بالا تفاوت داشتنشان عیب نیست، دو ابزار متفاوت است",
                    g_profileDaily.tpoPoc, g_profileDaily.tpoVah, g_profileDaily.tpoVal), clrAqua);
   if(g_profileDaily.amtValid)
   {
      ExpAddWrapped(StringFormat("Initial Balance (= اولین %d دقیقهٔ روز): %.2f تا %.2f — دامنه‌ای که «ارزش اولیه» را می‌سازد؛ شکست یک‌طرفهٔ آن + نبود بازگشت = روز روندی؛ برگشت به داخل = روز خنثی",
                    InpProfileIB_Minutes, g_profileDaily.ibLo, g_profileDaily.ibHi), clrMagenta);
      ExpAddWrapped(StringFormat("نوع روز: %s | نوع باز شدن: %s", AMTDayTypeName(g_profileDaily.dayType), AMTOpenTypeName(g_profileDaily.openType)), clrLime);
      ExpAddWrapped(StringFormat("وضعیت بازار: %s — روز باز: %.5f | بسته: %.5f | دامنه: %.5f–%.5f",
                    g_profileDaily.balanceNote, g_profileDaily.dayOpen, g_profileDaily.dayClose, g_profileDaily.dayLo, g_profileDaily.dayHi), clrLime);
      if(g_profileDaily.hasNakedPoc)
         ExpAddWrapped(StringFormat("Naked/Virgin POC: %.2f (روز %s) — POC یک سشن که از بسته‌شدن آن سشن تا الان لمس نشده؛ این سطح رفتار «آهنربایی» دارد، پس ورود برعکس درست روی آن پر‌ریسک است",
                       g_profileDaily.nakedPoc, TimeToString(g_profileDaily.nakedPocDay,TIME_DATE)), clrTomato);
   }
   ExpAddWrapped(StringFormat("HVN/LVN: %d گرهٔ پرحجم و %d گرهٔ کم‌حجم — HVN قیمتی است که بازار وقت زیادی آن‌جا گذرانده (مانع و آهنربا)، LVN قیمتی است که سریع عبور کرده (مسیر کم‌مقاومت). آستانه‌ها: HVN ≥ %.2f× و LVN ≤ %.2f× میانگین ردیف‌ها",
                 g_profileDaily.hvnCount, g_profileDaily.lvnCount, InpProfileHVN_Ratio, InpProfileLVN_Ratio), clrAqua);
   ExpAddWrapped("فیک/باطل: هر سه (IB · VA · HVN) با شکست و «پذیرش» (بسته‌شدن بیرون، نه فقط فتیله) بی‌اعتبار می‌شوند؛ اگر قیمت فقط فتیله بزند و داخل ببندد، سطح هنوز معتبر است", clrOrange);
   ExpAddWrapped(StringFormat("حجم کل پنجره: %d tick — صادقانه: MT5 روی فارکس/CFD فقط tick-volume می‌دهد؛ این تقریب است، نه حجم واقعی معاملات", g_profileDaily.totalVol), clrOrange);
   ExpAddWrapped("چرا اهمیت دارد: POC قیمتِ منصفانه است؛ ماندن قیمت داخل VA یعنی پذیرش (Balance) و خروج ردیفیِ قوی از VA یعنی Rejection و شروع روند", clrLime);
   ExpAddWrapped("فیک/باطل: برخورد لحظه‌ای به VAH/VAL معنای رد ندارد؛ بازگشت سریع داخل VA، خرابیِ شکست است", clrOrange);
   ExpAddWrapped("کنترل خودت: ردیف‌ها را با آستانهٔ InpProfileVA_Percent بازبینی کن؛ درصد پوشش Value Area فعلی همان تنظیم است", clrSilver);
}
...
void DrawBrooksRangeLayer()
{
   ...
   DrawBoxObj("ICTv13_BROOKS_TR", t1, g_brooksInfo.trHigh, t2, g_brooksInfo.trLow, clrSlateGray, false, true, STYLE_DOT, 1,
              StringFormat("Brooks Trading Range %.2f - %.2f | دوطرفه: bull/bear case هر دو زنده | وسط رنج (%.2f) مغناطیس است و شکست اغلب ناموفق است",
                           g_brooksInfo.trLow, g_brooksInfo.trHigh, g_brooksInfo.trMid));
   DrawHLineObj("ICTv13_BROOKS_TRMID", g_brooksInfo.trMid, clrSilver, STYLE_DOT, 1,
                StringFormat("Brooks Range 50%% (مغناطیس) = %.2f", g_brooksInfo.trMid));
   if(g_brooksInfo.mmValid)
      DrawHLineObj("ICTv13_BROOKS_MM", g_brooksInfo.mmD, clrSteelBlue, STYLE_DASHDOT, 1,
                   StringFormat("Brooks Measured Move (AB=CD) هدف D = %.2f | A=%.2f B=%.2f C=%.2f | ۱۰۰%% امتداد لگ اول",
...
void DrawQTLayer()
{
   ...
   if(g_qtDayTrueOpen>0.0 && g_qtDayTrueOpenT>0)
   {
      ...
      DrawHLineObj(nm,g_qtDayTrueOpen,clrViolet,STYLE_DASHDOT,1,
         StringFormat("Quarterly Theory — True Day Open (00:00 NY) = %s از %s | قیمت زیر = Discount برای لانگ، بالا = Premium برای شورت (True Open = بازِ Q2)",
   ...
   if(g_qtWkTrueOpen>0.0 && g_qtWkTrueOpenT>0)
   {
      ...
      DrawHLineObj(nm,g_qtWkTrueOpen,clrOrchid,STYLE_DASHDOT,1,
         StringFormat("Quarterly Theory — True Week Open (دوشنبه 00:00 NY) = %s | مرجع Premium/Discount هفتگی", DoubleToString(g_qtWkTrueOpen,_Digits)));
   }
}
...
void ExplainQT(bool week)
{
   ...
   ExpAddWrapped("چیست: Quarterly Theory (Trader Daye) هر چرخهٔ زمانی را به چهار ربع تقسیم می‌کند و انتظار دارد همان قالب AMD-X در هر ربع تکرار شود: Q1 Accumulation (رنج تنگ) → Q2 Manipulation (سوئپ/Judas) → Q3 Distribution (حرکت انبساطی) → Q4 Continuation یا Reversal. منبع: arongroups — Quarterly Theory", clrWhite);
   if(!week)
   {
      ExpAddWrapped(StringFormat("ربع فعلی روز: %s", QTPhaseToStr(g_qtDayPhase)), clrLime);
      ExpAddWrapped(StringFormat("True Day Open: %s (کندل ساعت 00:00 نیویورک) — قاعده: لانگ زیر True Open (Discount) و شورت بالای آن (Premium) ترجیح دارد",
                    (g_qtDayTrueOpen>0.0? DoubleToString(g_qtDayTrueOpen,_Digits):"هنوز ثبت نشده")),
                    g_qtDayTrueOpen>0.0?clrAqua:clrOrange);
      ExpAddWrapped("چه چیزی درست است / چه چیزی فیک: سوئپ Q1-رنج توسط Q2 = سیگنال اصلی؛ اگر Q2 سوئپی ساخت و برگشت، جهت Q3 همان جهت مخالف سوئپ است؛ اگر سوئپی نبود، چرخه طبق قالب نرفته و هیچ الزامی به معامله نیست — QT زمان‌بند است نه سیگنال", clrLime);
      ExpAddWrapped(StringFormat("ربع فعلی هفته: %s — هفته: دوشنبه Q1، سه‌شنبه Q2، چهارشنبه Q3، پنج‌شنبه/جمعه Q4 (روایت رایج؛ برخی منابع جمعه را خارج شمارش می‌دانند)", QTPhaseToStr(g_qtWeekPhase)), clrAqua);
   }
   else
   {
      ExpAddWrapped(StringFormat("True Week Open: %s (اولین کندل دوشنبه بعد از 00:00 نیویورک) — مرجع Premium/Discount هفتگی",
                    (g_qtWkTrueOpen>0.0? DoubleToString(g_qtWkTrueOpen,_Digits):"هنوز ثبت نشده")),
                    g_qtWkTrueOpen>0.0?clrAqua:clrOrange);
      ExpAddWrapped("نکتهٔ صادقانه: نسخه‌های متفاوتی از شمارش هفته وجود دارد (بعضی دوشنبه را آماده‌سازی می‌دانند و سه‌شنبه را Q1)؛ این اندیکاتور روایت اصلی Daye را نشان می‌دهد و آن را صریح می‌گوید", clrSilver);
   }
   ExpAddWrapped("اتصال به بقیهٔ سیستم: Q2 NY ≈ Killzone لندن + Judas، Q3 NY ≈ Killzone نیویورک AM؛ پس سوئپِ نقدینگی که همین سیستم در آن پنجره‌ها ثبت می‌کند دقیقاً همان رویداد Q2 است — دو اسم برای یک رویداد، نه دو محاسبه", clrAqua);
   ExpAddWrapped("کنترل خودت: ساعت نیویورک را از داشبورد بخوان؛ ربع فعلی باید با ساعت هم‌خوان باشد؛ خط True Open باید روی بازِ کندل 00:00 NY چارت باشد", clrSilver);
}
...
void DrawPhase12Layer()
{
   ...
   if(InpDetectTrendlineLiq && InpDrawTrendlines)
   {
      for(int i=0;i<ArraySize(g_trendlines);i++)
      {
         ...
         ObjectSetInteger(0,nm,OBJPROP_COLOR,g_trendlines[i].isHigh?clrTomato:clrMediumSeaGreen);
   ...
   if(InpDrawBestPOI && FindBestPOI(DIR_NONE, bp) && bp.top>bp.bottom)
   {
      ...
      DrawBoxObj(nm, bp.time, bp.top,
                 ...
                 bp.direction==DIR_BULL?clrDeepSkyBlue:clrOrangeRed, false, true, STYLE_DOT, 1,
...
bool IsLatinChar(ushort c)
{
   return (c < 0x0600);   // ASCII / لاتین / رقم / علائم
}
...
string RtlSafe(string s)
{
   int n=StringLen(s);
   if(n==0) return s;
   if(IsLatinChar(StringGetCharacter(s,0))) return s;   // خط با لاتین شروع شده: ترتیب درست است

   string latin="", persian="", cur="";
   bool curLatin=false, started=false;
   for(int i=0;i<n;i++)
   {
      ushort c=StringGetCharacter(s,i);
      bool isLat=IsLatinChar(c);
      if(!started){ curLatin=isLat; started=true; }
      if(isLat!=curLatin)
      {
         if(curLatin) latin  += cur+" ";
         else         persian+= cur;
         cur="";
         curLatin=isLat;
      }
   ...
   if(curLatin) latin+=cur; else persian+=cur;
   StringTrimRight(latin);
   StringTrimLeft(persian);
   if(StringLen(latin)==0) return persian;
   if(StringLen(persian)==0) return latin;
...
void ExpAdd(string text, color clr)
{
   ...
   // متن واقعی Unicode را به کنترل native می‌دهیم؛ تبدیل دستی RTL در OBJ_LABEL
   // باعث معکوس‌شدن فاصله و ترتیب کلمات می‌شد.
   g_expLines[n]=text;
   g_expLineColors[n]=clr;
}

// شکستن متن طولانی به خطوط کوتاه (تا در یک خط نریزد) (تا در یک خط نریزد)
void ExpAddWrapped(string text, color clr)
{
   ...
   if(StringLen(line)>0) ExpAdd(line, clr);
}
...
string HitTestExplainObject(int mx, int my)
{
   ...
   for(int i=0;i<total;i++)
   {
      ...
      if(dist<bestDist && dist<=tol*2.0){ bestDist=dist; best=nm; }
   }
   return best;
}
...
void ExplainLiquidity(const LiquidityObj &l)
{
   ...
   g_expTitle="LIQ "+LiqTypeLabel(l.type)+" #"+IdToStr(l.id);
   ExpAddWrapped(StringFormat("چیست: سطح نقدینگی %s در قیمت %s؛ دامنه: %s؛ تایم‌فریم منبع: %s",
                 highSide?"سمت خرید (BSL)":"سمت فروش (SSL)", PriceS(l.price),
                 l.scope==SCOPE_EXTERNAL?"خارجی/HTF":"داخلی/LTF", l.isHTF?EnumToString(InpHTF):EnumToString(PERIOD_CURRENT)), clrWhite);

   if(l.type==LIQ_PDH||l.type==LIQ_PDL||l.type==LIQ_PWH||l.type==LIQ_PWL)
      ExpAddWrapped(StringFormat("چرا تشکیل شد: سقف/کف دورهٔ قبلی (روزانه یا هفتگی) که قبل از زمان %s بسته شده بود — نقدینگی طبیعی بازار",
                    TimeToString(l.time,TIME_DATE|TIME_MINUTES)), clrAqua);
   else if(l.type==LIQ_EQH||l.type==LIQ_EQL)
   {
      ExpAddWrapped(StringFormat("چرا تشکیل شد: دو یا چند سویینگ تأییدشده در فاصلهٔ کمتر از تلورانس تنظیم‌شده (%.0f پوینت یا %.2f×ATR) قرار گرفتند **و یک پس‌رفت جداکننده بین‌شان بود** — استخر نقدینگی",
                    InpEQ_Tolerance_Points, InpEQ_ToleranceATR), clrAqua);
      ExpAddWrapped(StringFormat("شرط جدایی (منبع: LuxAlgo — Equal Highs/lows As Liquidity، مرحلهٔ ۲ «Require separation»): دو عضو باید یک سویینگ مخالف در میانه داشته باشند تا «دو تست مستقل» باشند، نه «یک سقف کشیده». عمق لازم حداقل برابر تلورانس است و با InpEQ_MinSeparationATR (×ATR) سخت‌تر می‌شود — مقدار فعلی %.2f",
                    InpEQ_MinSeparationATR), clrSilver);
   }
   else if(l.type==LIQ_SESSION_H||l.type==LIQ_SESSION_L)
      ExpAddWrapped(StringFormat("چرا تشکیل شد: رنج یک پنجرهٔ سشن بستهٔ قبل (مثلاً Asian Range) در ساعت %s به‌عنوان هدف نقدینگی ثبت شد",
                    TimeToString(l.time,TIME_DATE|TIME_MINUTES)), clrAqua);
   else
      ExpAddWrapped("چرا تشکیل شد: یک سوئینگ تأییدشده (پیوت با تأیید کندل‌های دو طرف) که به‌عنوان نقدینگی ثبت شده است", clrAqua);

   ExpAdd("وضعیت فعلی: "+(l.state==LSTATE_FRESH?"FRESH (دست‌نخورده)":
          (l.state==LSTATE_SWEPT?"SWEPT (جارو شده) در "+TimeToString(l.sweptTime,TIME_DATE|TIME_MINUTES)
                               :"INVALID (قیمت با بسته‌شدن از سطح عبور کرد و نقدینگی پذیرفته شد)")),
          l.state==LSTATE_FRESH?clrLime:(l.state==LSTATE_SWEPT?clrOrange:clrDimGray));

   ExpAddWrapped("اعتبار (شکست واقعی): اگر کندل کامل بسته بالای/زیر این سطح ببندد، سطح از حالت نقدینگی خارج و به مرز ساختاری تبدیل می‌شود (BOS در همان جهت)", clrLime);
   ExpAddWrapped("فیک (Sweep): اگر فقط فتیله از سطح بگذرد و کندل دوباره پشت آن بسته شود، این یک برداشت نقدینگی است، نه شکست؛ زمینهٔ حرکت مخالف می‌شود", clrOrange);
   ExpAddWrapped("چه چیزی آن را باطل می‌کند: بسته‌شدن کندل از سمت مخالف سطح (سطح دیگر هدف معتبر نیست)؛ یا اینکه خود Bias مالک تغییر کند", clrTomato);
   ExpAddWrapped(StringFormat("کنترل خودت: کندل %s را روی تایم‌فریم %s باز کن؛ باید بتوانی در آن محدوده دو سوئینگ/سقف‌کف تقریباً هم‌سطح را ببینی؛ قیمت سطح = %s",
                 TimeToString(l.time,TIME_DATE|TIME_MINUTES), EnumToString(PERIOD_CURRENT), PriceS(l.price)), clrSilver);
   ExpAddWrapped("برای ردیابی عددی: در فایل ICT_Assistant_Canonical_Explain.csv ردیف LIQ #"+IdToStr(l.id)+" را جستجو کن", clrSilver);
}

void ExplainStructureEvent(const StructureEvent &e)
{
   string typeTxt = (e.type==EVT_MSS)?"MSS (تغییر ساختار تأییدشده)":(e.type==EVT_CHOCH?"CHoCH (تغییر احتمالی)":"BOS (ادامهٔ روند)");
   g_expTitle="EVENT "+EventTypeToStr(e.type)+" #"+IdToStr(e.id)+" "+(e.direction==DIR_BULL?"BULLISH":"BEARISH");
   ExpAddWrapped(StringFormat("چیست: %s روی تایم‌فریم %s؛ زمان کندل تأیید: %s؛ قیمت سطح شکسته‌شده: %s",
                 typeTxt, EnumToString(e.isHTF?InpHTF:PERIOD_CURRENT), TimeToString(e.time,TIME_DATE|TIME_MINUTES), PriceS(e.price)), clrWhite);
   ExpAddWrapped("چرا تشکیل شد: کندل بسته‌شده سطح سویینگ #"+IdToStr(e.brokenSwingId)+" را رد کرد؛ سویینگ محافظت‌شدهٔ مقابل #"+IdToStr(e.protectedSwingId)+"؛ Displacement #"+IdToStr(e.displacementId)+"؛ Sweep #"+IdToStr(e.sweepId), clrAqua);

   if(e.type==EVT_MSS)
      ExpAddWrapped("معنی: تغییر ساختار با شاهد کامل (Sweep + Displacement + بسته‌شدن فراتر از سطح) — معتبرترین حالت برای شروع ستاپ", clrLime);
   else if(e.type==EVT_CHOCH)
      ExpAddWrapped("هشدار: CHoCH یعنی فقط جهت شکسته شده، ولی Displacement کافی ثبت نشده؛ این هنوز کاندید است و به‌تنهایی ستاپ نمی‌سازد", clrOrange);
   else
      ExpAddWrapped("معنی: ادامهٔ روند فعلی (شکست در جهت روند) — نه تغییر روند", clrAqua);

   ExpAddWrapped(StringFormat("شرط‌های عددی که ایندیکاتور اعمال کرده: بدنه/دامنه ≥ %.2f و دامنه/ATR ≥ %.2f برای Displacement؛ در غیر اینصورت ارتقا به MSS انجام نمی‌شود",
                 InpDisp_BodyRatio, InpDisp_RangeVsAvg), clrSilver);
   ExpAddWrapped("چه چیزی آن را باطل می‌کند: بسته‌شدن کندل از سطح محافظت‌شدهٔ مقابل #"+IdToStr(e.protectedSwingId)+"؛ یا ساخته‌شدن یک MSS مخالف بعدی", clrTomato);
   ExpAddWrapped(StringFormat("کنترل خودت: روی کندل %s وایسا؛ اگر کندل کامل بالای %s بسته شده، شکست درست ثبت شده؛ اگر فقط فتیله بوده، ثبت اشتباه است و باید گزارش کنی",
                 TimeToString(e.time,TIME_DATE|TIME_MINUTES), PriceS(e.price)), clrSilver);
   ExpAddWrapped("برای ردیابی عددی: دفتر رویداد را با شناسهٔ "+IdToStr(e.id)+" در ICT_Assistant_V13_Events_v1.csv جستجو کن", clrSilver);
}
...
{
   if(k==FVGK_IMPLIED)       return "IMPLIED (ناحیه بین میانهٔ فتیله‌ها)";
   if(k==FVGK_MICRO)         return "MICRO (گپ ریز روی تایم‌فریم پایین‌تر)";
   if(k==FVGK_VOL_IMBALANCE) return "VOLUME IMBALANCE (گپ بدنه‌ها با فتیلهٔ هم‌پوشان)";
   return "STANDARD (گپ سه‌کندلی high/low)";
}
...
{
   g_expTitle="ZONE "+FVGKindToStr(f.kind)+" #"+IdToStr(f.id)+" "+(f.direction==DIR_BULL?"BULLISH":"BEARISH");
   ExpAddWrapped(StringFormat("نام ICT همین گپ: %s — دو اسم برای یک محاسبه، نه دو محاسبه؛ اگر جایی BISI/SIBI جدا از FVG دیدی، همان FVG است با برچسب جهت‌دار",
                 FVGBisiSibiStr(f.direction)), clrAqua);   // فاز ۳۶
   ExpAddWrapped(StringFormat("چیست: %s؛ ناحیه %.2f تا %.2f؛ CE (میانه/۵۰ درصد) %.2f؛ زمان تشکیل: %s",
                 FVGKindToStr(f.kind), f.bottom,f.top,f.ce,TimeToString(f.time,TIME_DATE|TIME_MINUTES)), clrWhite);
   // تعریف هر نوع، جداگانه — منبع: LuxAlgo Library (Fair Value Gap / Implied FVG / Consequent Encroachment).
   if(f.kind==FVGK_STANDARD)
      ExpAddWrapped(StringFormat("چطور ساخته شد (Standard): فتیلهٔ کندل اول و سوم هم‌پوشانی ندارند — پایین ناحیه %.2f و بالای آن %.2f؛ کندل میانی Displacement #%s",
                    f.bottom, f.top, IdToStr(f.displacementId)), clrAqua);
   else if(f.kind==FVGK_IMPLIED)
      ExpAddWrapped(StringFormat("چطور ساخته شد (Implied): فرمول میانهٔ فتیله — پایین = میانهٔ فتیلهٔ بالای کندل اول، بالا = میانهٔ فتیلهٔ پایین کندل سوم؛ فتیله‌ها هم‌پوشانی دارند و گپ واقعی چاپ نشده؛ کندل میانی Displacement #%s",
                    IdToStr(f.displacementId)), clrAqua);
   else if(f.kind==FVGK_VOL_IMBALANCE)
      ExpAddWrapped(StringFormat("چطور ساخته شد (Volume Imbalance): گپ بین **بدنهٔ** کندل اول و سوم در حالی که فتیله‌ها هم‌پوشانی دارند؛ شاهد ضعیف‌تر از FVG واقعی است؛ کندل میانی Displacement #%s",
                    IdToStr(f.displacementId)), clrAqua);
   else
      ExpAddWrapped(StringFormat("چطور ساخته شد (Micro): گپ ریز روی تایم‌فریم %s؛ کندل میانی Displacement #%s",
                    EnumToString(f.tf), IdToStr(f.displacementId)), clrAqua);
   ExpAddWrapped(StringFormat("Causal بودن: %s",
                 f.causal?"بله (Displacement به Structure Event زنجیر شده)":"خیر (Displacement هنوز به شکست ساختار زنجیر نشده)"),
                 f.causal?clrAqua:clrOrange);
   ExpAdd("وضعیت: "+(f.invalidated?"INVALID/منقضی":(f.inverted?"iFVG (پولاریتی برعکس)":(f.ceTouched?"CE لمس شده (به‌قدر کافی پر)":(f.mitigated?"MITIGATED (فقط لبه لمس شده)":"FRESH (تازه)")))),
          f.invalidated?clrDimGray:(f.inverted?clrMediumOrchid:((f.ceTouched||f.mitigated)?clrGoldenrod:clrLime)));
   // فاز ۲۸ — تفکیک لمس لبه از رسیدن به CE (منبع: LuxAlgo — Consequent Encroachment).
   ExpAddWrapped((f.mitigated && !f.ceTouched)
                 ? "فرق مهم: تا این لحظه فقط لبهٔ ناحیه لمس شده؛ خط تصمیم، میانه (CE) است و هنوز به آن نرسیده — پس «پر شده» حساب نمی‌شود"
                 : (f.ceTouched
                    ? "فرق مهم: قیمت به CE (میانه) رسیده؛ بیشتر مدل‌ها همین را «به‌قدر کافی پر شده» می‌دانند"
                    : "هنوز هیچ لمسی ثبت نشده؛ ناحیه دست‌نخورده است"), clrSilver);
   ExpAddWrapped(StringFormat("فیلتر اهمیت اعمال‌شده: حداقل ارتفاع گپ = %.2f × ATR (تنظیم InpMinFVG_ATR)؛ ATR همین تحلیل %.2f — یعنی گپ‌های ریز از چارت حذف شده‌اند",
                 InpMinFVG_ATR, g_analysisATR), clrSilver);
   ExpAddWrapped("استفادهٔ درست: ورود روی پول‌بک به داخل ناحیه در جهت Bias؛ دقیق‌ترین ورود روی CE است (و اگر با OTE و ناحیهٔ OB هم‌پوشانی داشته باشد قوی‌تر است)", clrLime);
   ExpAddWrapped("لمس معتبر: ناحیه در همان کندل شکل‌گیری لمس‌شده حساب نمی‌شود؛ MITIGATED فقط از کندل بعد ثبت می‌شود (مرز گپ همیشه با کندل سوم لمس می‌شود)", clrSilver);
   ExpAddWrapped("فیک/بی‌اعتبار: بسته‌شدن کندل از سمت مخالف گپ (زیر ناحیه در صعودی) ناحیه را به iFVG تبدیل می‌کند؛ از آن به بعد دیگر ناحیهٔ ورود هم‌جهت نیست", clrOrange);
   if(!f.causal)
      ExpAddWrapped("هشدار کیفیت: این ناحیه به Displacement معتبر وصل نیست؛ ICT آن را ناحیهٔ درجه‌یک نمی‌داند و در این ایندیکاتور هم عمدتاً برای متن‌فریم استفاده نمی‌شود", clrOrange);
   ExpAddWrapped(StringFormat("انقضا: اگر تا %d کندل لمس/استفاده نشود، منقضی می‌شود (تنظیم فعلی)", InpFVG_ExpireBars), clrSilver);
   ExpAddWrapped(StringFormat("کنترل خودت: سه کندل اطراف %s را ببین؛ باید وسطی با بدنهٔ بزرگ باشد و گپ بین اول و سوم دیده شود؛ CE=%.2f",
                 TimeToString(f.time,TIME_DATE|TIME_MINUTES),(f.top+f.bottom)/2.0), clrSilver);
   ExpAddWrapped("برای ردیابی عددی: ردیف FVG #"+IdToStr(f.id)+" در ICT_Assistant_Canonical_Explain.csv", clrSilver);
}

void ExplainOB(const OBObj &o)
{
   g_expTitle="ZONE OB #"+IdToStr(o.id)+" "+(o.direction==DIR_BULL?"BULLISH":"BEARISH");
   ExpAddWrapped(StringFormat("چیست: اردر بلاک؛ ناحیه %.2f تا %.2f؛ نوع ناحیه: %s؛ زمان: %s",
                 o.bottom,o.top, InpOBUseFullCandleRange?"کل کندل":"فقط بدنه", TimeToString(o.time,TIME_DATE|TIME_MINUTES)), clrWhite);
   ExpAddWrapped("چرا تشکیل شد: آخرین کندل مخالف‌رنگ قبل از Displacement #"+IdToStr(o.displacementId)+" و همزمان با Structure Event #"+IdToStr(o.structureEventId)+" و Sweep #"+IdToStr(o.liquidityEventId)+" ثبت شد", clrAqua);
   ExpAdd("وضعیت: "+(o.state==OB_VALID?"VALID":(o.state==OB_MITIGATED?"MITIGATED":(o.state==OB_BROKEN?"BROKEN":(o.state==OB_BREAKER?"BREAKER":"INVALID")))),
          (o.state==OB_VALID)?clrLime:(o.state==OB_BREAKER?clrDeepSkyBlue:clrGoldenrod));
   ExpAddWrapped("زنجیرهٔ تبدیل به Breaker (سخت‌گیرانه): اردر بلاک معتبر + ناحیه‌ای که روی یک Sweep نقدینگی ساخته شده → بسته‌شدن از سمت مخالف (BROKEN) → ریتست در کندلی جدا و بعد از کندل شکست (BREAKER). اگر ناحیه با Sweep ساخته نشده باشد، حتی با شکست و ریتست هم Breaker نمی‌شود", clrWhite);
   ExpAddWrapped("لمس معتبر: ناحیه در همان کندل ثبت خودش MITIGATED حساب نمی‌شود؛ MITIGATED فقط از کندل بعد", clrSilver);
   ...
   ExpAddWrapped((o.scope==SCOPE_EXTERNAL)
      ? "نوع جایگاه: EXTERNAL (ERL) — این ناحیه روی مبدأ/اکسترمم لگ معامله‌گری نشسته؛ شکستش یعنی خروج از رنج و معمولاً ادامهٔ حرکت یا آغاز لگ تازه است — هدف بعدی معمولاً نقدینگی Internal (بازگشت به داخل لگ) خواهد بود"
      : "نوع جایگاه: INTERNAL (IRL) — این ناحیه داخل لگ باقی مانده؛ هدف کلاسیک ERL-to-IRL: بعد از مصرف نقدینگی بیرونی، قیمت برای پرکردن ناهنجاری‌های داخل لگ برمی‌گردد؛ ورود هم‌جهت روی ناحیهٔ Internal در Discount/Premium درست منطقی‌تر است",
      (o.scope==SCOPE_EXTERNAL)?clrGold:clrAqua);
   ExpAddWrapped("شرط استفاده: فقط وقتی ناحیه در جهت Bias مالک باشد و همراه با ناحیهٔ FVG/CE همان محدوده هم‌پوشانی داشته باشد؛ Solo و بدون ساختار استفاده نکن", clrLime);
   ExpAddWrapped("فیک/بی‌اعتبار: اگر ناحیه بدون تازه‌بودن و بارها لمس شود (MITIGATED) اعتبارش می‌رود؛ اگر کندل بسته از ناحیه عبور کند (BROKEN) دیگر اردر بلاک نیست", clrOrange);
   if(o.state==OB_INVALID)
      ExpAddWrapped("دلیل INVALID بودن ثبت این ناحیه: به Displacement و Structure Event معتبر وصل نشده بود؛ چنین ناحیه‌ای فقط برای شفافیت رسم می‌شود", clrOrange);
   ExpAddWrapped(StringFormat("کنترل خودت: کندل %s را ببین؛ باید کندل مخالف رنگ قبل از حرکت بزرگ باشد؛ ناحیهٔ ثبت‌شده: %.2f - %.2f",
                 TimeToString(o.time,TIME_DATE|TIME_MINUTES),o.bottom,o.top), clrSilver);
   ExpAddWrapped("برای ردیابی عددی: ردیف OB #"+IdToStr(o.id)+" در ICT_Assistant_Canonical_Explain.csv", clrSilver);
}

void ExplainRejection(const RejectionObj &r)
{
   g_expTitle="ZONE REJECTION #"+IdToStr(r.id)+" "+(r.direction==DIR_BULL?"BULLISH":"BEARISH");
   ExpAddWrapped(StringFormat("چیست: بلاک پس‌زدگی؛ ناحیه = فتیلهٔ غالب %.2f تا %.2f (نه کل کندل)؛ زمان: %s", r.bottom,r.top,TimeToString(r.time,TIME_DATE|TIME_MINUTES)), clrWhite);
   ExpAddWrapped("چرا تشکیل شد: در همان کندل یک سطح نقدینگی ثبت‌شده جارو (Sweep) شد و بیش از ۶۰ درصد دامنه را یک فتیله گرفت (بدنه کمتر از ۴۰ درصد) — یعنی قیمت نقدینگی را برداشت و برگشت", clrAqua);
   ExpAddWrapped("شرط ثبت در این ایندیکاتور: بدون Sweep یک سطح نقدینگی در همان کندل، هیچ Rejection Blockی ثبت نمی‌شود؛ لمس همان کندل سازنده هم TOUCHED حساب نمی‌شود", clrSilver);
   ExpAdd("وضعیت: "+RejectionStateToStr(r.rejectionState),
          r.rejectionState==REJECTION_FRESH?clrLime:(r.rejectionState==REJECTION_TOUCHED?clrGoldenrod:clrDimGray));
   ExpAddWrapped("استفادهٔ درست: به‌عنوان ناحیهٔ ورود فقط وقتی هم‌جهت با Bias و در آمیخته با Sweep/Displacement باشد؛ یک کندل پس‌زدگی تنها کافی نیست", clrLime);
   ExpAddWrapped("فیک/بی‌اعتبار: لمس شدن ناحیه (TOUCHED) تازگی را کم می‌کند و بسته‌شدن از سمت مخالف آن را بی‌اعتبار می‌کند", clrOrange);
...
void ExplainSessionBox(string kind, int dayBack)
{
   ...
   ExpAddWrapped(StringFormat("چیست: رنج همین پنجره؛ سقف %.2f و کف %.2f از %d کندل روی تایم‌فریم %s",
                 hi,lo,cnt,EnumToString(InpSessionSourceTF)), clrAqua);
   if(kind=="ASIA")
   {
      ExpAddWrapped("چرا مهم است: آسیا فاز انباشت (Accumulation) است؛ رنج آن هدف نقدینگی اصلی لندن می‌شود و بزرگ‌ترین حرکت روز معمولاً از برداشت همین نقدینگی شروع می‌شود", clrLime);
      ExpAddWrapped("چه انتظاری داشته باشیم در لندن: اگر فقط فتیله از سقف/کف آسیا بگذرد و برگردد = Judas Swing / سوئپ؛ اگر کندل بسته بیرون ببندد = شکست واقعی و تغییر رفتار", clrLime);
   ...
   {
      ExpAddWrapped("چیست: این یک پنجرهٔ زمانی است، نه سیگنال. قاعدهٔ ICT این است که در این یک ساعت، اگر نقدینگی مشخصی جارو شود و بعد جابه‌جایی (Displacement) و FVG/MSS در جهت Bias مالک ظاهر شود، ستاپ کیفیت بالاتری دارد", clrLime);
      ExpAddWrapped("چرا مهم است: این پنجره‌ها ساعاتی هستند که در آن‌ها الگوریتم/نقدینگی نهادی معمولاً حرکت جهت‌دار را شروع می‌کند؛ پس همان الگوی همیشگی را در زمانِ پراحتمال‌تر می‌بینی", clrLime);
      ExpAddWrapped("فیک/بی‌اعتبار: اگر داخل پنجره فقط رنج باشد و هیچ Sweep یا Displacement رخ ندهد، هیچ ستاپی معتبر نیست — خودِ ساعت دلیل ورود نیست؛ همچنین اگر Bias مالک مخالف باشد، حرکت داخل پنجره فقط ضد‌روند است", clrOrange);
      ExpAddWrapped("کنترل خودت: ساعت نیویورک شروع/پایان را روی داشبورد ببین و با محدودهٔ همین باکس مقایسه کن؛ اگر جابه‌جا بود، DST یا آفست بروکر را بررسی کن", clrSilver);
   }
   else
   {
      ExpAddWrapped("چرا مهم است: سقف/کف همین کیلیزون به‌عنوان نقدینگی کوتاه‌مدت ثبت می‌شود و سوئپ آن معمولاً نقطهٔ شروع حرکت جهت‌دار است", clrLime);
   ...
   ExpAddWrapped(StringFormat("کنترل خودت: روی چارت محدودهٔ %s تا %s را ببین؛ اگر با ساعت نیویورک نمی‌خواند، آفست بروکر (InpBrokerGMTOffsetOverrideHours) یا DST را باید تنظیم کرد؛ داشبورد همان آفست تشخیص‌داده‌شده را در ردیف clock نشان می‌دهد",
...
void ExplainMTFLevel(int index, bool isHigh)
{
   ...
   ExpAddWrapped(StringFormat("چیست: سطح حفاظت‌شدهٔ %s روی تایم‌فریم %s؛ نقش در سلسله‌مراتب: %s؛ زمان داده: %s",
                 isHigh?"سقف":"کف", tf, g_mtfContext[index].role,
   ...
   ExpAddWrapped(StringFormat("جهت محاسبه‌شده: %s (خارجی) و %s (داخلی)",
                 DirToStr(shownExt), DirToStr(g_mtfContext[index].internalDirection)), clrAqua);
   if(index==0)
      ExpAddWrapped("قاعدهٔ سخت: این تایم‌فریم مالک Bias است. هیچ تایم‌فریم پایین‌تری (M15/M5/M2/M1) اجازه ندارد این Bias را عوض کند؛ فقط می‌تواند تأیید یا رد کند و READY را مسدود کند", clrGold);
   else
      ExpAddWrapped("قاعده: این تایم‌فریم نقش Context/Setup/Confirmation دارد؛ هم‌جهت بودن آن با Bias شرط لازم است و تضاد آن فقط باعث مسدود شدن READY می‌شود (نه تغییر Bias)", clrLime);
   if(g_mtfConflict)
      ExpAddWrapped("وضعیت فعلی: تضاد فعال است — "+g_mtfConflictReason, clrOrange);
   ExpAddWrapped(StringFormat("کنترل خودت: روی تایم‌فریم %s بازش کن و ببین جهت ساختار (سقف‌ها/کف‌های تأییدشده) با چیزی که اینجا نوشته هم‌خوان است یا نه", tf), clrSilver);
}
...
void ExplainSetup()
{
   ...
   if(g_setup.active)
   {
      ExpAddWrapped(StringFormat("ستاپ فعال: جهت %s | Entry %s | SL %s | TP1 %s | TP2 %s | TP3 %s | R:R واقعی 1:%.2f",
                    g_setup.dir==DIR_BULL?"LONG":"SHORT", PriceS(g_setup.entry), PriceS(g_setup.sl),
                    PriceS(g_setup.tp1), PriceS(g_setup.tp2), PriceS(g_setup.tp3), g_setup.rr), clrLime);
      ExpAddWrapped(StringFormat("زنجیرهٔ اثبات‌شدهٔ این ستاپ: %s | ناحیهٔ ورود: %s (FVG #%s / OB #%s) | DOL %.2f",
                    g_setup.chainText, g_setup.zoneSource, IdToStr(g_setup.fvgId), IdToStr(g_setup.obId), g_currentDOL.price), clrAqua);
      ExpAddWrapped(StringFormat("ریسک واقعی: Entry تا SL = %.2f (%.2f ATR) | بافر SL: %s",
                    g_setup.risk, (g_analysisATR>0.0? g_setup.risk/g_analysisATR : 0.0), g_setup.slSource), clrAqua);
      ExpAddWrapped(StringFormat("R:R واقعی از فاصله‌ها حساب شده، نه از ورودی: TP3 %s منهای Entry %s بخش بر ریسک %.2f = 1:%.2f | TP1 1:%.2f | TP2 1:%.2f",
                    PriceS(g_setup.tp3), PriceS(g_setup.entry), g_setup.risk, g_setup.rrTP3, g_setup.rrTP1, g_setup.rrTP2), clrLime);
      ExpAddWrapped("چرا READY شد (دقیقاً همان شرط‌هایی که کد چک می‌کند): Bias مالک تأیید شده؛ DOL هم‌جهت با فاصلهٔ کافی موجود است؛ تضاد تایم‌فریم نیست؛ H1/M15/M5 هم‌جهت‌اند؛ M2/M1 تأیید اجرایی داده‌اند؛ یک چرخهٔ اثبات‌شدهٔ Sweep→Displacement→شکست ساختار هم‌جهت و تازه وجود دارد؛ لگ واقعی H4 معتبر و هم‌جهت با Bias است؛ ناحیهٔ ورود داخل باند OTE همان لگ است؛ SL با ATR و StopsLevel ساخته شده و صفر نیست؛ و R:R واقعی از حداقل ورودی کمتر نیست", clrLime);
      ExpAddWrapped("محدودیت شناخته‌شدهٔ این نسخه: مدیریت پوزیشن و حجم معامله محاسبه نمی‌شود؛ ناحیهٔ M5 فقط وقتی جایگزین می‌شود که وسط آن داخل باند OTE باشد؛ چرخهٔ عمر ستاپ فقط **یک** ستاپ در هر لحظه را پیگیری می‌کند", clrOrange);
      ExpAddWrapped("چه چیزی آن را باطل می‌کند: تغییر یا تضاد Bias مالک؛ از دست رفتن DOL هم‌جهت؛ حرکت لگ به لگ مخالف؛ بسته‌شدن قیمت فراتر از مرز OTE به سمت مخالف؛ یا مصرف‌شدن/منقضی‌شدن ناحیه", clrTomato);
   }
   ExpAddWrapped("فاز Exhaustion: "+ExhaustionStateToStr(g_exhaustion.state)+" با امتیاز "+IntegerToString(g_exhaustion.score)+"/"+IntegerToString(g_exhaustion.maxScore)+" — این هشدار است و به‌تنهایی Bias H4 را تغییر نمی‌دهد", clrOrange);
   ExpAddWrapped(StringFormat("فاز برگشت تأییدشده: %s | دروازهٔ سطح محافظت‌شده: %s | Smart Money Reversal %d از %d — برگشت فقط با بسته‌شدن فراتر از سطح محافظت‌شدهٔ خارجی اعلام می‌شود و Exhaustion هرگز Bias را عوض نمی‌کند",
   ...
   // فاز ۱۲ (#۳ #۹ #۴۷ #۶۷ #۶۸)
   ExpAddWrapped(StringFormat("فاز ۱۲ — مدل ورود: %s (%s) | امتیاز کیفیت: %d از ۱۰ (حداقل %d) | تفکیک: %s",
                 EntryModelToStr(g_setup.entryModel), g_setup.modelReason, g_setup.quality, InpMinQualityScore,
                 (StringLen(g_setup.qualityText)>0? g_setup.qualityText : "—")), g_setup.entryModel==MODEL_NONE?clrOrange:clrLime);
   ExpAddWrapped(StringFormat("ساختار داخلی همان تایم‌فریم مالک (%s): %s %s | فاز روند: %s با سن %d کندل HTF (%s)",
                 EnumToString(InpHTF), DirToStr(g_htfInternalDir),
                 (StringLen(g_htfInternalReason)>0? "— "+g_htfInternalReason : ""),
                 TrendPhaseToStr(g_trend.phase), g_trend.ageBars, g_trend.phaseReason), clrAqua);
   ExpAddWrapped(StringFormat("POI انتخاب‌شده: %s با امتیاز %d (شناسه #%s) | نقدینگی مورب: %s | رنج: %s",
                 PoiKindToStr(g_setup.poiKind), g_setup.poiScore, IdToStr(g_setup.poiId),
                 g_trendlineBuilt? "فعال" : g_trendlineReject,
                 g_rangeOk? StringFormat("فعال: %.2f تا %.2f (%.1f ATR) با %d و %d برخورد",
                    g_rangeLow, g_rangeHigh, (g_analysisATR>0.0? g_rangeHeight/g_analysisATR:0.0),
                    g_rangeLowTouches, g_rangeHighTouches) : g_rangeReject), clrAqua);
   ExpAddWrapped(StringFormat("سطوح IPDA: %s", g_ipdaNote), g_ipdaOk?clrAqua:clrOrange);
   if(g_exhaustion.reason!="") ExpAddWrapped("شواهد: "+g_exhaustion.reason, clrSilver);
   if(!g_setup.active)
   {
      ExpAddWrapped("دلیل اینکه هنوز READY نیست: "+g_setup.status, clrOrange);
      ExpAddWrapped("معنی کدها: WAITING_H4_BIAS یعنی Bias مالک هنوز تأیید نشده؛ WAITING_DOL یعنی سطح نقدینگی هم‌جهت با فاصلهٔ کافی پیدا نشد؛ WAITING_MTF_CONFLICT تضاد تایم‌فریم‌ها؛ WAITING_H1_CONTEXT یا WAITING_M15_CONTEXT یا WAITING_M5_SETUP هم‌جهت نبودن کانتکست؛ WAITING_M2_CONFIRMATION و WAITING_M1_CONFIRMATION تأیید اجرایی نشده؛ WAITING_ICT_CYCLE هیچ چرخهٔ اثبات‌شده‌ای در جهت Bias نیست؛ WAITING_PROVEN_SWEEP_MSS چرخه هست ولی Sweep و Displacement کامل نیست؛ WAITING_FRESH_CYCLE چرخه قدیمی‌تر از پنجرهٔ مجاز است؛ WAITING_DEALING_LEG لگ واقعی معتبر نیست؛ WAITING_LEG_DIRECTION آخرین لگ مخالف Bias است؛ WAITING_RETRACE ناحیهٔ ورود نیست؛ WAITING_DISCOUNT_OTE یا WAITING_PREMIUM_OTE قیمت/ناحیه داخل باند OTE همان لگ نیست؛ WAITING_ZONE_GEOMETRY هندسهٔ ناحیه نامعتبر؛ WAITING_SL_INVALID فاصلهٔ ورود تا SL صفر یا منفی می‌شد؛ WAITING_DOL_DIRECTION هدف در جهت معامله نیست؛ WAITING_RR_LOW R:R واقعی از حداقل کمتر است؛ WAITING_ENTRY_MODEL هیچ مدل ورودی شرط‌هایش کامل نشد؛ WAITING_QUALITY_LOW امتیاز کیفیت از حداقل کمتر است؛ SETUP_ENGINE_DISABLED موتور ستاپ خاموش است", clrSilver);
      ExpAddWrapped("برای رسیدن به READY چه لازم است (دقیقاً به همین ترتیب کد): ۱) Bias مالک روی H4 تأیید شود ۲) DOL هم‌جهت با حداقل فاصله ۳) بدون تضاد تایم‌فریمی ۴) H1/M15/M5 هم‌جهت ۵) M2/M1 تأیید اجرایی ۶) چرخهٔ اثبات‌شدهٔ Sweep+Displacement+شکست ساختار هم‌جهت و تازه ۷) لگ واقعی H4 معتبر و هم‌جهت ۸) ناحیهٔ ورود (اول M5) داخل باند OTE ۹) SL با بافر ATR/StopsLevel و R:R واقعی کافی ۱۰) DOL آن‌سوی ورود", clrLime);
   }
   ...
   if(InpTrackSetupLifecycle)
   {
      ...
      ExpAddWrapped(StringFormat("چرخهٔ عمر ستاپ: %s | مسلح‌شده در %s | SL پیگیری %s | TP1 پیگیری %s | شمارنده‌ها: مسلح %d / باطل %d / TP1 %d",
                    lifeColor, (g_setupLifeArmTime>0? TimeToString(g_setupLifeArmTime,TIME_DATE|TIME_MINUTES):"—"),
      ...
      if(g_setupLifeState=="INVALIDATED")
         ExpAddWrapped(StringFormat("ابطال با عبور *بستهٔ* قیمت از SL رخ داده است: %s | کندل %s | close %s | SL %s — فتیله کافی نیست، فقط close ملاک است",
                       g_setupLifeReason, TimeToString(g_setupInvalidTime,TIME_DATE|TIME_MINUTES),
                       PriceS(g_setupInvalidPrice), PriceS(g_setupLifeSL)), clrTomato);
      else if(g_setupLifeState=="TP1_HIT")
         ExpAddWrapped("هدف اول (۱R) لمس شد و پیگیری این ستاپ بسته شد: "+g_setupLifeReason, clrLime);
      else if(g_setupLifeState=="TRACKING")
         ExpAddWrapped("این ستاپ در حال پیگیری است: ابطال فقط وقتی ثبت می‌شود که یک کندل *بستهٔ* چارت فراتر از SL بسته شود؛ لمس فتیله ابطال نیست", clrOrange);
      else
         ExpAddWrapped("هنوز هیچ ستاپ آماده‌ای مسلح نشده است؛ با READY شدن ستاپ، پیگیری شروع می‌شود و در فایل ICT_Assistant_Canonical_SetupLifecycle.csv ثبت می‌گردد", clrSilver);
   }
   ExpAddWrapped(StringFormat("آفست زمانی: بروکر %+d دقیقه، استاندارد (زمستان) %+d دقیقه، قاعدهٔ DST %s، حالت محاسبهٔ تاریخی: %s — پنجره‌های سشن با آفست همان لحظهٔ تاریخی ساخته می‌شوند، نه آفست امروز",
                 g_serverGMTOffsetSeconds/60, g_brokerStdOffsetSeconds/60,
                 BrokerDSTRuleToStr(g_brokerDSTRule),
                 InpUseHistoricalBrokerOffset?"تاریخی (DST لحظه‌ای)":"آفست جاری"), clrAqua);
   ExpAddWrapped("کنترل خودت: چیزی که ایندیکاتور می‌گوید را با سه چیز چک کن — کندل تأیید، قیمت سطح، و شناسه‌ها؛ اگر ناسازگار بود همان عدد را با Pivot دستی مقایسه کن؛ ردیف SETUP را در ICT_Assistant_Canonical_Explain.csv ببین", clrSilver);
}
...
void ExplainLocation(string label)
{
   ...
   if(!g_leg.valid)
   {
      ExpAddWrapped("لگ واقعی هنوز معتبر نیست؛ دلیل: "+g_leg.reject, clrOrange);
      ExpAddWrapped("معنی دلایل: H4_DATA_NOT_READY دادهٔ کافی برای پیوت‌یابی نیست؛ NOT_ENOUGH_ALTERNATING_PIVOTS سقف/کف تأییدشدهٔ متناوب کافی نیست؛ ZERO_RANGE دامنهٔ لگ صفر است؛ LEG_TOO_SMALL اندازهٔ لگ از حداقل نسبت به ATR کمتر است", clrSilver);
      ExpAddWrapped("کنترل خودت: دو پیوت آخر H4 را روی چارت ببین؛ اگر فاصلهٔ پیوت‌ها کم یا نامنظم است، تایم‌فریم بالاتر برای رمزگذاری لگ مناسب‌تر است", clrSilver);
      return;
   }

   string dirFa = (g_leg.dir==DIR_BULL)?"صعودی":"نزولی";
   ExpAddWrapped(StringFormat("چیست: دامنهٔ معامله‌گری (Dealing Range) از همان لگ واقعی ساخته شده — از پیوت آغاز %s در %.2f تا پیوت پایان %s در %.2f",
                 (g_leg.dir==DIR_BULL?"کف":"سقف"), g_leg.startPrice,
                 (g_leg.dir==DIR_BULL?"سقف":"کف"), g_leg.endPrice), clrWhite);
   ExpAddWrapped(StringFormat("این دو نقطه دقیقاً در %s و %s هستند؛ همان دو پیوت تأییدشدهٔ H4 — جهت لگ: %s | اندازهٔ لگ: %.2f (%.2f ATR)",
                 TimeToString(g_leg.startTime,TIME_DATE|TIME_MINUTES), TimeToString(g_leg.endTime,TIME_DATE|TIME_MINUTES),
   ...
   ExpAddWrapped(StringFormat("مقادیر روی همین لگ: تعادل EQ %.2f | باند OTE %.2f تا %.2f (%.0f%% تا %.0f%%) | نقطهٔ طلایی %.2f (%.1f%%)",
                 eq, oteLow, oteHigh, InpOTE_Low*100.0, InpOTE_High*100.0, golden, InpOTE_Golden*100.0), clrAqua);
   ExpAddWrapped(StringFormat("قیمت تحلیل‌شده (close همان کندل بستهٔ موتور): %.2f → در نتیجه در سمت %s نسبت به EQ است",
                 g_analysisClose, (g_analysisClose>=eq?"Premium (مثبت)":"Discount (تخفیف)")), clrWhite);
   // فاز ۳۶: توضیح Price Delivery (برچسب لگ روی چارت = همین مفهوم)
   ExpAddWrapped(StringFormat("Price Delivery: قیمت همیشه از یک نقدینگی به نقدینگی دیگر «تحویل» می‌شود؛ این لگ %s از %s شروع شده و الان در %s لگ است — بازگشت به EQ/OTE = فرصت ورود هم‌جهت (Mitigation)؛ عبور قاطع از اکسترمم لگ = تحویل به نقدینگی بعدی و لگ تازه",
                 dirFa, TimeToString(g_leg.startTime,TIME_DATE|TIME_MINUTES),
                 (g_analysisClose>=eq?"Premium (نیمهٔ بالایی)":"Discount (نیمهٔ پایینی)")), clrLime);

   if(label=="GOLDEN")
      ExpAddWrapped(StringFormat("خاصیت نقطهٔ طلایی: %.2f وسط همان باند OTE است و جدش ۷۰٫۵ درصد ریتریس لگ است؛ ورود در همین نقطه کم‌ترین ریسک و بیشترین تطابق با لگ را دارد", golden), clrLime);
   else
      ExpAddWrapped("خاصیت این سطح: EQ نصف لگ است؛ بالای آن Premium و پایین آن Discount. در Bias صعودی ورود فقط از Discount و در باند OTE، و در Bias نزولی ورود فقط از Premium و در باند OTE", clrLime);

   ExpAddWrapped("توجه: این باند فقط وقتی در ستاپ استفاده می‌شود که جهت لگ با Bias مالک یکی باشد؛ لگ مخالف دلیل رد شدن ستاپ است، نه دلیلی برای ورود برعکس", clrOrange);
   ExpAddWrapped("کنترل خودت: یک فیبوی دستی روی همان دو نقطه (همان دو زمان بالا) بیانداز؛ باید ۶۲ و ۷۰٫۵ و ۷۹ همین اعداد را بدهد. اگر فرق داشت، همین شناسهٔ لگ را گزارش کن", clrSilver);
}
...
void ExplainDOL()
{
   ...
   if(!g_hasDOL)
   {
      ExpAddWrapped("فعلاً DOL معتبری وجود ندارد (واکنش به Bias مالک نشده یا سطح هم‌جهت پیدا نشد)", clrOrange);
      if(g_dolRejectReason!="") ExpAddWrapped("دلیل واقعی: "+g_dolRejectReason, clrSilver);
      return;
   }
   ExpAddWrapped(StringFormat("چیست: سطح نقدینگی هم‌جهت Bias که قیمت به سمت آن کشیده می‌شود — %s در %.2f",
                 g_currentDOL.typeName, g_currentDOL.price), clrWhite);
   ExpAddWrapped(StringFormat("چرا این سطح: %s", g_currentDOL.narrative), clrAqua);
   ExpAddWrapped(StringFormat("فاصلهٔ واقعی تا هدف: %.2f ATR (%.2f قیمت) | در همین سطح حداقل فاصله %.2f ATR اعمال می‌شود تا R:R ناچیز رد شود",
                 g_currentDOL.distATR, MathAbs(g_currentDOL.price-g_analysisClose), InpDOL_MinRoomATR), clrAqua);
   ExpAddWrapped(StringFormat("وضعیت سطح: دست‌نخورده (FRESH) = %s | همان سلسله‌مراتب: %s",
                 g_currentDOL.sweepStateOk?"بله":"خیر", g_currentDOL.hierarchyOk?"External (بالاتر) تأیید شده":"Internal"), clrLime);
   ExpAddWrapped("اشتباه رایج: انتخاب دورترین سطح به بهانهٔ سود بیشتر — R:R واقعی وقتی خراب می‌شود که هدف خیلی نزدیک باشد، نه خیلی دور", clrOrange);
   ExpAddWrapped("کنترل خودت: قیمت هدف را با سقف/کف روز یا هفتهٔ قبل روی چارت مقایسه کن؛ اگر آن سطح با یک خط دیگر Registry یکی نیست، همین شناسه را گزارش کن", clrSilver);
}
...
void ExplainTrendline(long id)
{
   ...
   if(idx<0)
   {
      ...
      ExpAddWrapped("این خط در رجیستری Trendline پیدا نشد؛ گزارش کن", clrOrange);
      return;
   }
   ...
   ExpAddWrapped(StringFormat("چیست: نقدینگی مورب — خطی که %s تأییدشدهٔ تایم‌فریم %s را وصل می‌کند. استاپ‌ها پشت همین خط جمع می‌شوند",
                 g_trendlines[idx].isHigh?"دو سقف":"دو کف", EnumToString(g_trendlines[idx].tf)), clrWhite);
   ...
   ExpAddWrapped(StringFormat("شرط ابطال: بسته‌شدن کندل با فاصلهٔ بیش از %.2f ATR فراتر از خط — یعنی نقدینگی این خط برداشته شده و خط دیگر معتبر نیست",
                 InpTrendlineBreakATR), clrTomato);
   if(g_trendlines[idx].swept)
      ExpAddWrapped(StringFormat("جارو شد: در کندل %s فتیله از خط عبور کرد ولی کندل به همان سمت خط بسته شد — قاعدهٔ Sweep مورب (فتیلهٔ فراتر + بسته‌شدن برگشتی). قیمت خط در آن کندل %s بود و به‌عنوان %s در Registry ثبت و «مصرف‌شده» علامت خورد",
                    TimeToString(g_trendlines[idx].sweptTime,TIME_DATE|TIME_MINUTES),
                    DoubleToString(g_trendlines[idx].sweptPrice,_Digits),
                    g_trendlines[idx].isHigh? "LIQ_TRENDLINE_H (Buy-Side)" : "LIQ_TRENDLINE_L (Sell-Side)"), clrYellow);
   else
   ...
   ExpAddWrapped(StringFormat("کنترل خودت: دو آنکری که نام برده شده را روی چارت ببین؛ شیب آن‌ها باید با قاعدهٔ سمت نقدینگی بخواند (سقف‌های نزولی = Buy-Side بالای خط، کف‌های صعودی = Sell-Side زیر خط) و تعداد سویینگ روی خط باید حداقل %d باشد (تنظیم InpTrendlineMinTouches)",
...
void ExplainBestPOI()
{
   ...
   {
      ExpAddWrapped("رجیستری POI خالی است یا با ورودی InpBuildPOIRegistry خاموش شده", clrOrange);
      return;
   }
   ExpAddWrapped(StringFormat("چیست: رجیستری یکپارچهٔ نقاط مورد علاقه — %d ناحیه با نوع، جهت، محدوده، زمان، تایم‌فریم مالک و امتیاز عددی. منبع: FVG causal، Order Block، Breaker، Mitigation Block، Rejection Block، نقدینگی مورب و مرزهای رنج", ArraySize(g_poi)), clrWhite);
   if(FindBestPOI(g_htfBias, bp))
   {
      ExpAddWrapped(StringFormat("بهترین POI هم‌جهت Bias: %s | جهت %s | ناحیه %s تا %s | زمان %s | TF %s | امتیاز %d | شناسه #%s",
                    PoiKindToStr(bp.kind), DirToStr(bp.direction), DoubleToString(bp.bottom,_Digits),
                    DoubleToString(bp.top,_Digits), TimeToString(bp.time,TIME_DATE|TIME_MINUTES),
                    EnumToString(bp.tf), bp.score, IdToStr(bp.id)), clrLime);
   }
   else ExpAddWrapped("هیچ POI هم‌جهتی با Bias فعلی در رجیستری نیست", clrOrange);
   if(FindBestPOI(DIR_NONE, bp))
      ExpAddWrapped(StringFormat("بالاترین امتیاز کل رجیستری: %s با امتیاز %d در جهت %s",
                    PoiKindToStr(bp.kind), bp.score, DirToStr(bp.direction)), clrAqua);
   ExpAddWrapped(StringFormat("تفکیک امتیاز: پایه ۲۰ | Breaker +۱۲ | FVG +۸ | OB +۶ | Trendline/Range +۵ | Mitigation +۴ | Rejection +۳ | هم‌جهتی با Bias +۱۰ | نشستن روی اکسترمم لگ +۶ | منهای کهنگی (هر %d کندل یک امتیاز، حداکثر ۱۰) | Nested Mitigation +۴: ناحیهٔ HTF که POI هم‌جهت LTF داخلش نشسته (منبع: SMC — Nested/Multi-timeframe Mitigation)",
                 MathMax(1,InpPOI_AgeDecayBars)), clrSilver);
   ExpAddWrapped(StringFormat("ستاپ فعلی: مدل %s | امتیاز کیفیت %d از ۱۰ (حداقل ورودی %d) | %s",
                 EntryModelToStr(g_setup.entryModel), g_setup.quality, InpMinQualityScore,
                 g_setup.qualityText), g_setup.active?clrLime:clrOrange);
   ExpAddWrapped("امتیاز کیفیت هیچ‌وقت جای دروازهٔ ساختار را نمی‌گیرد؛ فقط ستاپ‌های ضعیف را رد می‌کند. POI هم فقط انتخاب ناحیه و توضیح را یکسان می‌کند", clrTomato);
}
...
void ExplainReversal()
{
   ...
   ExpAddWrapped(StringFormat("چیست: برگشت تأییدشده فقط یک معنا دارد — کندل بستهٔ %s فراتر از سطح محافظت‌شدهٔ خارجی *بسته* شود. فتیله کافی نیست؛ بسته‌شدن لازم است (#۸/#۷۱)", EnumToString(InpHTF)), clrWhite);

   if(!InpEnableReversalGate)
   {
      ExpAddWrapped("دروازهٔ برگشت با ورودی InpEnableReversalGate خاموش است؛ در این حالت هیچ برگشتی تأیید نمی‌شود و تنها هشدار Exhaustion نمایش داده می‌شود", clrOrange);
      return;
   }
   if(!g_reversal.armed)
   {
      ExpAddWrapped("الان سطح محافظت‌شدهٔ خارجی فعالی وجود ندارد؛ وضعیت: "+g_reversal.state, clrOrange);
      ExpAddWrapped("دلیل واقعی: "+g_reversal.reason, clrSilver);
      ExpAddWrapped("معنی کدها: WAITING_H4_BIAS هنوز Bias مالک تأیید نشده؛ WAITING_PROTECTED_LEVEL آخرین رویداد H4 سویینگ محافظ نداده یا با سقف InpMaxSwings از رجیستری حذف شده؛ WAITING_HTF_DATA دادهٔ بستهٔ HTF کافی نیست", clrSilver);
      ExpAddWrapped("کنترل خودت: آخرین رویداد H4 را روی چارت ببین؛ همان رویداد باید یک سویینگ مقابل را به‌عنوان محافظ معرفی کرده باشد — همان سویینگ سطح این دروازه است", clrSilver);
      return;
   }
   ...
   ExpAddWrapped(StringFormat("جهت برگشتی که این سطح تأیید می‌کند: %s | Bias لحظهٔ سنجش: %s | کندل سنجیده‌شده: %s با close %s",
                 DirToStr(OppositeDir(g_reversal.snapBias)), DirToStr(g_reversal.snapBias),
                 TimeToString(g_reversal.snapBarTime,TIME_DATE|TIME_MINUTES), DoubleToString(g_reversal.snapBarClose,_Digits)), clrAqua);

   if(g_reversal.confirmed)
   {
      ExpAddWrapped(StringFormat("برگشت تأییدشده: جهت %s | کندل تأیید %s | close تأیید %s | %d کندل چارت از تأیید گذشته | رویداد #%s",
                    DirToStr(g_reversal.dir), TimeToString(g_reversal.confirmedTime,TIME_DATE|TIME_MINUTES),
                    DoubleToString(g_reversal.confirmedClose,_Digits), g_reversal.barsSince, IdToStr(g_reversal.eventId)), clrLime);
      ExpAddWrapped(StringFormat("Smart Money Reversal (#۷۰): %s — %s", g_reversal.smr?"تأییدشده":"تأیید نشده", g_reversal.smrReason),
                    g_reversal.smr?clrLime:clrOrange);
      ExpAddWrapped(StringFormat("چهار شاهد شمارش‌شده (حداقل %d لازم است): ۱) Sweep نقدینگی مقابل در %d کندل HTF قبل از شکست ۲) Displacement زنجیرشدهٔ هم‌جهت ۳) ناحیهٔ هم‌جهت (FVG causal معتبر یا Order Block معتبر) ۴) هم‌جهتی ساختار داخلی/پایین‌تر با جهت برگشت",
                    InpSMR_MinScore, InpSMR_SweepLookbackBars), clrSilver);
   }
   else
   {
      ExpAddWrapped("هنوز برنگشته: "+g_reversal.reason, clrOrange);
      ExpAddWrapped("نکتهٔ مهم: Exhaustion هرگز به‌تنهایی Bias را عوض نمی‌کند. تنها همین دروازه (بسته‌شدن فراتر از سطح محافظت‌شدهٔ خارجی) برگشت را تأیید می‌کند", clrTomato);
   }

   ExpAddWrapped("چه چیزی آن را باطل می‌کند: ساخت یک رویداد ساختاری جدید روی H4 که سطح محافظت‌شده را جابه‌جا کند؛ در آن حالت شاهد برگشت قبلی دیگر «تازه» نیست و فقط تاریخچه می‌ماند. توجه: برگشت تأییدشده به‌تنهایی معنای «فروش حالا» نیست؛ فقط می‌گوید ساختار محافظت‌شده در جهت مخالف بسته شده است", clrTomato);
   ExpAddWrapped("کنترل خودت: کندل بستهٔ H4 را باز کن؛ اگر close آن فراتر از این سطح است، تأیید درست است و اگر فقط فتیله بوده، ثبت اشتباه است و باید گزارش کنی. ردیف REVERSAL را در ICT_Assistant_Canonical_Explain.csv و ردیف‌های زمانی را در ICT_Assistant_Canonical_Reversal_Diag.csv ببین", clrSilver);
}
...
void BuildExplanation(string objName)
{
   ...
   if(StringFind(body,"SWEEP_")==0)
   {
      ...
      {
         ExplainLiquidity(l);
         ExpAddWrapped(StringFormat("Sweep ثبت‌شده در %s — نقدینگی برداشته شد؛ برای تبدیل شدن به ستاپ، نیاز به Displacement و شکست ساختار در جهت مخالف Sweep است",
                       TimeToString(l.sweptTime,TIME_DATE|TIME_MINUTES)), clrOrange);
      }
      return;
   }
   ...
   if(StringFind(body,"MTF_")==0)
   {
      ...
      ExpAdd("سطح MTF | تایم‌فریم: "+tag, clrWhite);
      return;
   }
   ...
   {
      g_expTitle="AMD قیمتی (Power of Three)";
      ExpAddWrapped(g_amdPriceNote==""? "رنج آسیا هنوز ثبت نشده؛ A/M/D قیمتی فعلاً قابل بیان نیست": g_amdPriceNote, clrWhite);
      ExpAddWrapped("تفکیک زمانی از قیمتی: نگاشت سشنی (آسیا=Accumulation، لندن=Manipulation، NY=Distribution) فقط زمان‌بندی است؛ این بخش رفتار واقعی قیمت را می‌سنجد: سوئپ بیرون رنج آسیا = M؛ حرکت انبساطی مخالف سوئپ = D — منبع: ICT Power of Three و همان AMD-X در Quarterly Theory", clrAqua);
      return;
   }
   ...
   if(StringFind(body,"SD_")==0)
   {
      ...
      g_expTitle="S/D"; ExpAddWrapped("این ناحیه در رجیستری پیدا نشد (حذف‌شدهٔ FIFO)", clrOrange);
      return;
   }
   ...
   // هیچ آبجکتی نباید بی‌توضیح بماند. اگر شناسهٔ نام آبجکت با Registry
   // هم‌خوان نبود، صادقانه همین را بنویس تا کاربر جای خرابی را ببیند.
   if(ArraySize(g_expLines)==0)
   {
      g_expTitle="OBJECT "+body;
      ExpAddWrapped(StringFormat("چیست: این آبجکت روی چارت رسم شده ولی توضیح تخصصی آن در Registry پیدا نشد؛ نام کامل آن: ICTv13_%s", body), clrOrange);
      ExpAddWrapped("دلیل ممکن — اول: شناسهٔ داخل نام آبجکت با شناسهٔ داخلی Registry هم‌خوان نیست (مثلاً بریده‌شدن شناسهٔ ۶۴ بیتی). دوم: مرجع این آبجکت از رجیستری کران‌دار بیرون رفته است (سقف‌های InpMaxFVG/InpMaxOB/InpMaxEvents و...). اگر آبجکت به‌سبک frozen دیده می‌شود، حالت دوم است و باگی نیست؛ ولی اگر آبجکت زنده است و توضیح ندارد، حالت اول است و همان نام را گزارش کن", clrTomato);
      ExpAddWrapped("کنترل خودت: نام آبجکت را از لیست آبجکت‌ها (Ctrl+B) بردار و ببین همان عددی را دارد که در ICT_Assistant_Canonical_Explain.csv نوشته شده یا نه", clrSilver);
   }

   // فاز ۱۴: اگر این آبجکت به‌سبک frozen نگه داشته شده، **بالای** پنل بگو چرا،
   // تا کاربر خاکستری/نقطه‌چین بودن را با «اشتباه محاسبه» اشتباه نگیرد.
   if(ArraySize(g_expLines)>0 && IndexInNameList(g_frozen,objName)>=0)
   {
      int n=ArraySize(g_expLines);
      ArrayResize(g_expLines,n+1);
      for(int i=n;i>0;i--) g_expLines[i]=g_expLines[i-1];
      g_expLines[0]=StringFormat("FROZEN HISTORY: این آبجکت دادهٔ معتبر دارد ولی از پنجرهٔ نمایش فعلی بیرون افتاده است (سقف %d ناحیه / %d سطح) و بی‌صدا حذف نشد — فقط رنگ خاکستری و خط نقطه‌چین گرفت تا با اشیای زنده قاطی نشود. اگر بعداً داخل پنجرهٔ نمایش برگردد، رنگ واقعی و خط اصلی‌اش خودکار برمی‌گردد",
                                 InpMaxDrawnZones, InpMaxDrawnLevels);
      ArrayResize(g_expLineColors,n+1);
...
void PersistFVGDictionary()
{
   ...
   if(h==INVALID_HANDLE) return;
   FileWrite(h,"Id","Dir","Time","AgeBars","Top","Bottom","Causal",
             "Mitigated","Inverted","Invalidated","TouchTime","BarsToTouch","BirthBarTouch");
...
void RenderRiskStrip()
{
   ...
   string txt=StringFormat("Bias: %s  |  ریسک برگشت: %s (%d)  |  سطح: %s [%s] %s ATR  |  لگ %.0f%%",
             DirToStr(g_htfBias), g_rrLabel, g_rrScore, g_rrLevel, g_rrLevelState,
             DoubleToString(g_rrLevelDist,2), g_rrLegProg);
   if(g_hasDOL)
      txt+=StringFormat("  |  هدف: %s %s ATR", g_currentDOL.typeName, DoubleToString(g_rrDolDist,2));
   if(g_barSweptTowardBias)
      txt+="  |  SFP: نقدینگی سمت Bias جارو شد";
...
void RenderDashboard()
{
   ...
   DashRow("revrisk", StringFormat("ریسک برگشت: %s (%d) | نزدیک‌ترین سطح: %s [%s] %s ATR | لگ %.0f%% | هدف %s %s ATR%s",
             ...
             (g_barSweptTowardBias? " | SFP: نقدینگی سمت Bias جارو شد":"")),
             g_rrScore>=50? InpColorBear : (g_rrScore>=25? InpColorNeutral : InpColorBull));
                      ...
                      const double &c[], int shift, double atrValue)
{
   ...
   {
      g_exhaustion.reason="Bias مالک H4 هنوز تأیید نشده است";
      return;
   }
   ...
   if(reversalFresh)
   {
      ...
      g_exhaustion.reason=StringFormat("برگشت تأییدشده: کندل بستهٔ HTF با close %s فراتر از سطح محافظت‌شدهٔ خارجی بسته شد (Bias قبلی %s → جهت برگشت %s)؛ %d کندل چارت از تأیید گذشته است",
      ...
      g_exhaustion.reason+=g_reversal.smr
         ? (" | Smart Money Reversal تأییدشده: "+g_reversal.smrReason)
         : (" | Smart Money Reversal تأیید نشده: "+g_reversal.smrReason);
      g_exhaustion.reason+=" | این وضعیت هم Bias را نمی‌نویسد؛ مالک Bias همان موتور ساختار H4 است";
      return;
   }
   ...
   if(available<2)
   {
      g_exhaustion.reason="برای سنجش Exhaustion دادهٔ بستهٔ کافی وجود ندارد";
      return;
   }
   ...
   if(opposingClose){ g_exhaustion.score++; g_exhaustion.reason+="بسته‌شدن مخالف Bias؛ "; }
   if(opposingInternal){ g_exhaustion.score++; g_exhaustion.reason+="هشدار ساختار داخلی مخالف؛ "; }
   if(nearDOL){ g_exhaustion.score++; g_exhaustion.reason+="نزدیکی به DOL؛ "; }
   ...
   if(StringLen(g_exhaustion.reason)==0)
      g_exhaustion.reason="شواهد کافی برای ضعف روند دیده نشد؛ Bias H4 بدون تغییر است";
   if(microReversal)
      g_exhaustion.reason+=" این چرخش فقط داخلی است؛ برای تغییر Bias، شکست ساختار خارجی H4 لازم است.";
   else
      g_exhaustion.reason+=" این وضعیت هشدار است و به‌تنهایی Bias H4 را تغییر نمی‌دهد.";
}
...
void SelfTestWrite()
{
   ...
   if(h==INVALID_HANDLE)
   {
      PrintFormat("ICT PHASE37 | خطا: نوشتن گزارش آزمون رفتاری ناموفق بود (err=%d)",GetLastError());
      return;
   }
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   {
      int maxScan = ArraySize(h) - InpSwingLeft - InpSwingRight - 12;
