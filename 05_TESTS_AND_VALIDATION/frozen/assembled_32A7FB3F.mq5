...
// این ورودی‌ها در **انتهای** لیست اضافه شدند تا اندیس ورودی‌های قبلی عوض نشود.
input group "== Phase 47: pending scenario (در انتظار بسته شدن کندل، بدون ریپنت) =="
input bool   InpEnablePendingScenario = true;  // محاسبهٔ سناریوی در انتظار (خاموش = تحلیل دست‌نخورده می‌ماند)
input bool   InpShowPendingLine       = true;  // نمایش ردیف تک‌خطی روی چارت
input int    InpPendingLineY          = 30;    // فاصلهٔ ردیف از لبهٔ بالا (پیکسل؛ نوار ریسک در ۲ است)
input int    InpPendingLineWidth      = 700;   // عرض پایهٔ ردیف (به طول متن و عرض چارت محدود می‌شود)
...
string g_pendLastText      = "";      // کارایی: متن عوض نشده = هیچ ObjectSet/Redraw
bool   g_pendLineOn        = false;
...
void DrawFamiliesLayer()
{
   ...
   {
      DrawHLineObj("ICTv13_PROF_POC", g_profileDaily.poc, clrGold, STYLE_SOLID, 1, "POC | حجم تیک | "+g_profileNote);
      ...
      if(g_profileDaily.tpoValid)
      {
         ...
         DrawHLineObj("ICTv13_PROF_TVAH", g_profileDaily.tpoVah, clrKhaki, STYLE_DOT, 1, "TPO VAH | ناحیهٔ ارزش زمان‌محور");
         DrawHLineObj("ICTv13_PROF_TVAL", g_profileDaily.tpoVal, clrKhaki, STYLE_DOT, 1, "TPO VAL | ناحیهٔ ارزش زمان‌محور");
      }
...
void ExplainDispatchObject(string objName)
{
   ...
   // فاز ۴۷ — ردیف سناریوی در انتظار (PENDING). مثل نوار ریسک یک ردیف
   // فقط‌خواندنی است؛ توضیحش از همان ماژولی می‌آید که متن ردیف را می‌سازد،
   // تا هیچ محاسبهٔ موازی و هیچ دو روایتی از یک عدد وجود نداشته باشد.
   if(StringFind(body,"PEND")==0)
   {
      ExplainPendingScenario();
      return;
   }
...
{
   g_expTitle="SIGNAL STRIP — درجهٔ سیگنال و ریسک برگشت";
   ...
   ExpAdd("reverse-risk report: ICT_Assistant_Canonical_ReverseRisk_<symbol>.csv", clrSilver);
...
void RenderRiskStrip()
{
   ...
   // Phase 42: Latin-first readout labels lead, pure Persian follows after each
   // separator. Phase 46 puts the fitted grade first, because that is the header
   // the user asked for (the old risk index is kept but is no longer the lead).
   string txt=StringFormat("GRADE: %s  |  SCORE: %.2f  |  BIAS: %s  |  WIN: %.1f%%  |  درجهٔ ورود هم‌جهت  |  ریسک برگشت: %s (%d)  |  سطح: %s [%s]  |  فاصله %.2f برابر میانگین دامنه  |  لگ %.0f درصد",
             g_grade, g_gradeScore, DirToStr(g_htfBias), g_gradeWin,
             g_rrLabel, g_rrScore, g_rrLevel, g_rrLevelState,
             DoubleToString(g_rrLevelDist,2), g_rrLegProg);
   if(g_hasDOL)
      txt+=StringFormat("  |  هدف: %s  |  فاصله %.2f برابر میانگین دامنه", g_currentDOL.typeName, DoubleToString(g_rrDolDist,2));
   if(g_barSweptTowardBias)
                ...
                const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   RenderPendingLine();             // فاز ۴۷: ردیف در انتظار، زیر نوار ریسک
   PersistPhase14Diagnostics();
   return rates_total;
}
//+------------------------------------------------------------------+
...
//--------------------------------------------------------------------
// ردیف تک‌خطی. قاعدهٔ RTL فاز ۴۲: تنها واژهٔ لاتینِ ردیف («PENDING») سر ردیف
// می‌آید و بقیه فارسی است تا خط یک قطعهٔ یکپارچه بماند.
//--------------------------------------------------------------------
string PendingScenarioLine()
{
   if(!InpShowPendingLine) return "";
   if(g_pendState=="OFF") return "";
   if(g_pendState=="NO_BIAS" || g_pendState=="NO_GUARD" || g_pendState=="NO_ATR")
      return StringFormat("PENDING | %s", PendStateFa(g_pendState));

   string magnet=(g_pendMagnetPrice>0.0)
                 ? StringFormat(" | مقصد: %s %s در %.2f برابر دامنه",
                                g_pendMagnetTypeFa, DoubleToString(g_pendMagnetPrice,_Digits), g_pendMagnetDistATR)
                 : " | مقصد نقدینگی: سطح جارونشده‌ای در جهت بایاس نیست";

   if(g_pendState=="CONFIRMED_NOW")
      return StringFormat("PENDING | برگشت همین کندل بسته تأیید شد%s", magnet);

   return StringFormat("PENDING | %s | ماشه: %s | فاصله %.2f برابر دامنه | نرخ گذشته %.1f درصد از %d نمونه%s",
                       PendStateFa(g_pendState), g_pendTriggerFa, g_pendGuardDistATR,
                       g_pendRate, g_pendRateN, magnet);
}

//--------------------------------------------------------------------
// رسم ردیف — همان مسیر تأییدشدهٔ فارسی (OBJ_EDIT با متن خام)، دقیقاً زیر نوار
// ریسک. مثل نوار: اگر متن عوض نشده باشد هیچ ObjectSet و ChartRedraw نمی‌زند.
//--------------------------------------------------------------------
void RenderPendingLine()
{
   string txt=PendingScenarioLine();
   if(StringLen(txt)==0)
   {
      if(g_pendLineOn)
      {
         ObjectDelete(0,"ICTv13_PEND_BG");
         ObjectDelete(0,"ICTv13_PEND_TXT");
         g_pendLineOn=false; g_pendLastText="";
      }
      return;
   }
   if(g_pendLineOn && txt==g_pendLastText) return;
   g_pendLastText=txt;

   int chartW=(int)ChartGetInteger(0,CHART_WIDTH_IN_PIXELS,0);
   int w=InpPendingLineWidth;
   int need=(int)(StringLen(txt)*InpExplainFontSize*0.62)+24;
   if(need>w) w=need;
   if(chartW>20 && w>chartW-10) w=chartW-10;
   int lineH=InpExplainFontSize+11;

   string bg="ICTv13_PEND_BG";
   if(ObjectFind(0,bg)<0) ObjectCreate(0,bg,OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,bg,OBJPROP_CORNER,CORNER_LEFT_UPPER);
   ObjectSetInteger(0,bg,OBJPROP_XDISTANCE,2);
   ObjectSetInteger(0,bg,OBJPROP_YDISTANCE,InpPendingLineY);
   ObjectSetInteger(0,bg,OBJPROP_XSIZE,w);
   ObjectSetInteger(0,bg,OBJPROP_YSIZE,lineH+6);
   ObjectSetInteger(0,bg,OBJPROP_BGCOLOR,clrBlack);
   ObjectSetInteger(0,bg,OBJPROP_BORDER_TYPE,BORDER_FLAT);
   ...
   ExplainEditRow("ICTv13_PEND_TXT", 8, InpPendingLineY+3, w-12, lineH,
                  RenderLine(txt,InpExplainRenderMode),
                  clrAqua,
                  InpExplainFontSize);
   g_pendLineOn=true;
...
//--------------------------------------------------------------------
void ExplainPendingScenario()
{
   g_expTitle="PENDING — آنچه در انتظار بسته شدن کندل است";
   ...
   ExpAdd("object name: ICTv13_PEND_TXT (read-only row, not draggable)", clrSilver);
}
