...
## بازآرایی — مونولیت ۱۱٬۰۶۸ خطی به اسکلت + ۳۰ ماژول (۲۰۲۶-۰۹-۲۰)

**مسئله:** خواندن و ویرایش یک فایل ۱۱ هزار خطی برای هر تغییر گران بود و مرز خانواده‌ها در متن گم می‌شد.

**راه‌حل:** فایل `.mq5` به یک **اسکلت** تبدیل شد (خطوط ۱–۳۲: `#property` و دو `#define`؛ بدنه فقط `#include`) به‌همراه ۳۰ ماژول `.mqh` در `01_CANONICAL_CANDIDATES/modules/` — یک فایل برای هر خانوادهٔ استراتژی، به‌صورت **برش‌های پیوسته و به همان ترتیب اصلی**. چون ترتیب اعلان‌ها دست‌نخورده ماند، هیچ forward declaration و هیچ جابه‌جایی متغیر سراسری لازم نشد و شمارهٔ خط‌ها نیز دقیقاً حفظ شد.

| دروازه | نتیجه |
|---|---|
| `tools/Verify-ModuleSplit.ps1` | **PASS=11 FAIL=0** — بازچینی ۶۴۵٬۲۹۳ بایت؛ هش `13A34E6B…71E2`؛ و با `-ReferencePath` روی مونولیت واقعیِ خارج‌شده از git: **صفر بایت متفاوت** |
| `tools/Sync-And-Compile-Canonical.ps1` | پیش از هر بیلد برش را می‌سنجد و در صورت انحراف **از بیلد امتناع می‌کند**؛ اسکلت + ۳۰ ماژول را به پکیج و پوشهٔ MT5 آینه و هر فایل را با هش تأیید می‌کند |
| Build | **0 errors, 0 warnings** — `.ex5` تازه در `…\MQL5\Indicators\khaleq\newICT\` |
| رگرسیون ۱۹ ابزار | **۱۸ سبز**، بدون تغییر نسبت به قبل از بازآرایی |

**بدهی باز و صادقانه:** تنها ابزار قرمز `Validate-CanonicalEventLedger` است و دلیلش کهنه‌بودن فایل runtime (۱۵ سپتامبر) است، نه کد. با اولین reload روی چارت، آن فایل بازنویسی می‌شود و ابزار باید سبز شود.

جزئیات و لنگرهای کد: مستندات فنی پروژه بند ۴‑۰‑۳۹.

---

## فاز ۴۰ — رفع «پریدن آبجکت‌ها با تعویض تایم‌فریم» (۲۰۲۶-۰۹-۲۰)

**نشانه:** بدون هیچ خطای کامپایلی، با هر تعویض تایم‌فریم چارت خالی می‌شد.

**ریشه (از ژورنال ترمینال):** بلافاصله پس از هر attach، `array out of range in '06_MultiTimeframe.mqh' (246,30)` — خط ۲۴۶ = `v[shift]`. علت: `CopyTickVolume` فقط به تعداد کندل‌های **آمادهٔ** پر می‌کند و سری تازه‌سوییچ‌شده ناقص است، پس `ArraySize(v) < need` می‌شود؛ خطای runtime کل پاس `OnCalculate` را می‌کشد، بازسازی انجام نمی‌شود، `RedrawChartObjects()` صدا زده نمی‌شود و چون `OnDeinit` قبلاً آبجکت‌ها را پاک کرده بود ⇒ چارت خالی.

| محل | اصلاح |
|---|---|
| `UpdateWyckoff` | مرز `shift` با `volCopied` (طول واقعی کپی حجم) سنجیده می‌شود |
| `UpdateProfile` | حلقه با `bars = min(copiedV, ArraySize(h), ArraySize(l))` |
| `OnCalculate` | `avail = min(o,h,l,c,atrBuf)` و `maxScan` از `avail` (بافر ATR روی TF تازه) |
| کش پنجرهٔ پروفایل | `copied = min(copied, cH, cL, cO, cC)` |
| `OnDeinit` | **نشت هندل:** `g_atrHandle` آزاد می‌شود (تعویض TF = `OnDeinit`+`OnInit`) |

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `.ex5` تازه در `…\khaleq\newICT\` |
| `Verify-ModuleSplit` | OK — مرجع به `25D59CEC…` بازتنظیم شد (تغییر منطق عمدی، با diff بیرونی مستند در مستندات فنی پروژه بند ۴‑۰‑۴۰) |
| `tools/Validate-Phase40.ps1` | **منبع: PASS=7 FAIL=0** · **Runtime: بخش B تا یک reload روی build جدید قرمز می‌ماند** |

**بدهی باز:** بخش B (شاهد ژورنال) تا وقتی اندیکاتور روی چارت با build جدید reload نشود سبز نمی‌شود. ابزار بر اساس **ترتیب** حکم می‌دهد: خطای build قدیم نباید build فعلی را محکوم کند.

---

## فاز ۴۱ (۲۰۲۶‑۰۹‑۲۰) — خوانا شدن متن فارسی پنل آموزشی (رفع «فارسی با انگلیسی قاطی»)

| مورد | وضعیت |
|---|---|
| ریشه | قاعدهٔ UBA/P2-P3 روی کنترل بومی: واژهٔ لاتین وسط جملهٔ فارسی، جمله را به دو قطعهٔ جابه‌جا می‌شکند |
| چرا سه هفته برگشت | قاعده از قبل در سرصفحهٔ `25_Explain.mqh` نوشته و در `RtlSafe()` پیاده شده بود، ولی **هیچ‌جا صدا زده نمی‌شد** (کد مرده) |
| اصلاح ۱ | `RtlSafe()` توکن‌محور بازنویسی شد: فقط واژهٔ دارای حرف لاتین **بعد** از واژهٔ فارسی به ابتدای خط می‌رود؛ ارقام دست‌نخورده (عدد EN با قاعدهٔ W داخل قطعهٔ RTL می‌ماند) |
| اصلاح ۲ | گارد در `ExpAdd()` **واقعاً اجرا می‌شود** (سوییچ `InpExplainLatinFirst`، پیش‌فرض روشن) |
| اصلاح ۳ | `ExpEmitLine()`: اگر جابه‌جایی خط را بلندتر کرد، بلوک لاتین به خط خودش می‌رود |
| اصلاح ۴ | `RenderLine` حالت ۰: جهت پایه با U+200F قطعی می‌شود |
| اصلاح ۵ | متن آموزشی خانوادهٔ FVG بازنویسی شد: هیچ واژهٔ انگلیسی وسط جملهٔ فارسی نیست، عنوان فقط لاتین، برچسب‌های `FRESH/MITIGATED/iFVG` → معادل فارسی |
| بدهی صادقانه | ۲۵۵ رشته در دیگر خانواده‌ها هنوز لاتین وسط فارسی دارند و فقط با گارد زمان‌اجرا خوانا می‌شوند |

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `.ex5` تازه در `…\khaleq\newICT\` (۲۰۲۶‑۰۹‑۲۰ ۲۰:۵۲:۲۶) |
| `Verify-ModuleSplit` | OK — مرجع به `B22B2294…` بازتنظیم شد (۱۱٬۲۰۳ خط)، با diff بیرونی ۸‑هانکی در مستندات فنی پروژه بند ۴‑۰‑۴۱ |
| `Validate-PersianRender` | **PASS=43 FAIL=0** (۸ کیس قاعدهٔ `RtlSafe` با پاسخ دستی‌نویس + سیم‌کشی پنل + گارد ثابت متن FVG) |
| ابزارهای تازه | `tools/Assemble-Canonical.ps1` (بازچینی بایت‌به‌بایت برای diff) · `tools/Map-Modules.ps1` (بازتولید جدول خطوط ماژول‌ها) |
| رگرسیون | ۲۱ ابزار اعتبارسنج، همه سبز |

## فاز ۴۲ (۲۰۲۶‑۰۹‑۲۰) — هیچ واژهٔ انگلیسی داخل جملهٔ فارسی نمی‌ماند (بستن بدهی فاز ۴۱)

بدهی صریح فاز ۴۱ («۲۵۵ رشتهٔ دیگر فقط با گارد زمان‌اجرا خوانا می‌شوند») بسته شد.

| مورد | وضعیت |
|---|---|
| قاعدهٔ حاکم | یک واژهٔ لاتین **بعد** از یک واژهٔ فارسی، جمله را به دو قطعهٔ جابه‌جا می‌شکند. پس یا اصلاً لاتین نیست، یا برچسب لاتین **ابتدای ردیف** می‌آید و بعدش فقط فارسی خالص |
| صفت‌های تازه | `TfFa` · `DirFa` · `LiqTypeFa` · `LiqTypeCode` · `ObStateFa` · `ObKindFa` · `RejectionStateFa` · `ExhaustionStateFa` · `TrendPhaseFa` · `PoiKindFa` · `EntryModelFa` · `RTMEventFa` · `RTMEventCode` · `AMTDayTypeFa` · `AMTOpenTypeFa` · `SDKindFa` · `WyckoffEventFa` · `WyckoffEventCode` · `QTPhaseFa` · `SetupStatusFa` · `LegRejectFa` · `FVGKindFa` |
| فاز Wyckoff | نام فاز از حرف لاتین (`فاز A`) به نام فارسی (`فاز الف/ب/ج/د/هـ`) رفت، چون این رشته وسط جمله درج می‌شود |
| کدهای وضعیت | کد ستاپ لاتین **خالص** می‌مانند و در ردیف جدا چاپ می‌شوند (`WAITING_QUALITY_LOW (score 3 of 10)`) — دیگر متن فارسی داخل کد نیست |
| لایهٔ موتور | ۷۹ رشتهٔ `reason`/`note`/`why`/`narrative`/`breakdown`/`status` در ۱۴ ماژول بازنویسی شد (همان ردیف‌هایی که پنل به‌عنوان «دلیل واقعی» و «چرا این سطح» چاپ می‌کند) |
| ابزار لازم | `tools/Strip-Literals.ps1` — برای اثبات این‌که تغییر، فقط متن است نه منطق |

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `.ex5` در `…\khaleq\newICT\` (۲۰۲۶‑۰۹‑۲۰ ۲۲:۳۳:۳۶)، hash `6ECE6FCE…54C6` |
| `Validate-PersianRender` | **PASS=50 FAIL=0** — از جمله معیار تازهٔ **`no-literal-in-the-whole-source-puts-latin-after-persian`** که تمام واحد ترجمه را می‌سنجد، نه فقط توابع آموزشی |
| پوشش سنجش | ۲۵ تابع آموزشی · **۳۶۶ ردیف** پنل · بلندترین ردیف ۲۱۸ نویسه · خارج از سقف ۲۶۰: **۰** |
| اثبات دست‌نخوردن منطق | `Strip-Literals` همهٔ رشته‌ها، کامنت‌ها و فضای خالی بی‌معنا را حذف می‌کند؛ diff باقی‌مانده فقط ردیف‌های پنل/readout و یک تعویض برچسب `EnumToString(InpHTF)` → `TfFa(InpHTF)` است — هیچ شرط، حساب، ایندکس بافر یا نوشتن رجیستری تفاوت ندارد |
| `Verify-ModuleSplit` | OK — مرجع به `F6453447…` بازتنظیم شد (۱۱٬۶۲۹ خط) |
| رگرسیون | ۱۶ اعتبارسنج `PASS/FAIL`، همه `FAIL=0` · `TimeBase` ۰ اختلاف · `MTF-Hierarchy` PASSED · `V13EventLedger` PASSED |

## فاز ۴۳ (۲۰۲۶‑۰۹‑۲۱) — وارونگی گپ: جهت، رنگ، CSV و پنل یک چیز می‌گویند

**درخواست:** «ناسازگاری وارونه‌سازی FVG را رفع کن: جهت و رنگ و متن CSV و پنل باید یک چیز بگویند».

ریشهٔ باگ یک محاسبهٔ غلط نبود؛ **یک فیلد با دو معنی** بود: تابع وارونگی، `g_fvgs[].direction` را بازنویسی می‌کرد، پس از آن لحظه یک فیلد هم «جهت تولد» بود و هم «نقش فعلی» و هر سطح مصرف‌کننده بسته به زمان خواندن چیز دیگری می‌گفت.

| مورد | وضعیت |
|---|---|
| جهت تولد | **تغییرناپذیر** شد؛ وارونگی فقط `inverted` و `invertedTime` را ثبت می‌کند |
| نقش فعلی | یک مشتق واحد: `FVGActiveDir(f)` و برای OB `OBActiveDir(o)` روی `polarityFlipped` |
| گزارش | یک رشتهٔ مشترک `FVGPolarityReport(f)` → `BULLISH` یا `BULLISH -> BEARISH (iFVG)` برای پنل، ستون `Role` در CSV و ردیف `Explain.csv` |
| سطح‌های سوییچ‌شده | رنگ باکس · انتخاب ناحیهٔ ستاپ · رجیستری POI (FVG و OB) · دروازهٔ برگشت · برچسب ریسک · تیتر پنل FVG/OB · nested mitigation |
| خوانندگان عمدی جهت تولد | زنجیرهٔ Displacement · خود شرط وارونگی · شاهد فرسودگی · `DetectFVG` (همه کامنت‌گذاری و در ابزار قفل شده) |
| آزمون رفتاری | ۷ ردیف تازه روی دادهٔ ساختگی که همان `UpdateFVG_Lifecycle` زنده را اجرا می‌کند |
| ابزار تازه | `tools/Validate-Phase43.ps1` — قفل «جهت تولد هرگز نوشته نمی‌شود + همهٔ سطوح از یک مشتق + یک رشتهٔ گزارش» |
| ابزار به‌روزشده | `Validate-Phase36` (شرط nested به نقش فعلی) · `Validate-Phase37` (فهرست سناریوها + سه چک B20–B22) |

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `.ex5` در `…\khaleq\newICT\` (۲۰۲۶‑۰۹‑۲۱ ۲۳:۴۵:۱۳) |
| `Validate-Phase43` | **PASS=29 FAIL=0 PENDING=2** — بخش ۵ روی فایل زندهٔ تشخیصی، `DirBirth` را با رقم آخر شناسه و `ActiveDir` را با قطب مقابل می‌سنجد؛ تا reload، PENDING می‌دهد (نه سبز دروغین) |
| شاهد قبل/بعد (بدون چارت) | فایل قبلی: **۱۴۰ از ۱۴۰** ردیف وارونه تناقض داشتند (`Dir` مخالف جهت رمزگذاری‌شده در شناسه) و ۰ از ۱۰ غیروارونه. تفکیک: استاندارد ۲۲/۲۲ · ضمنی ۵/۵ · ریز ۸۲/۸۲ · ناترازی بدنه ۳۱/۳۱ |
| آزمون خود ابزار | `fvg_diag_phase43.fixture.csv` سبز · نسخهٔ دست‌کاری‌شده‌اش قرمز (پیام دقیق ردیف خطادار) |
| `Verify-ModuleSplit` | OK — مرجع به `37C28223…` بازتنظیم شد (۱۱٬۸۴۱ خط) با diff بیرونی که فقط هانک‌های مورد نظر را نشان داد |
| رگرسیون | ۲۱ ابزار، همه `exit 0` — به‌جز `Validate-Phase37` بخش B که تا یک reload، ردیف‌های تازهٔ گزارش را ندارد (PENDING واقعی، نه شکست کد) |

## فاز ۴۴ (۲۰۲۶‑۰۹‑۲۱) — مسیر کلیک: هر آبجکت با نام و دلیل جواب می‌دهد

**درخواست:** «با کلیک روی هر آبجکت، پنل باید قطعی و بدون خطا نام و دلیل همان آبجکت را نشان بدهد — مسیر کلیک را از صفر تست و قفل کن».

| مورد | وضعیت |
|---|---|
| شاهد قبل (فایل زندهٔ `Explain.csv`) | ۹۴ آبجکت؛ تنها `ICTv13_RSTRIP_TXT` به پیام جایگزین می‌افتاد و هر شاخه‌ای که مرجعش از رجیستری بیرون افتاده بود **بی‌صدا** برمی‌گشت (پنل خالی) |
| ساختار سه‌لایه | `ExplainDispatchObject` (تشخیص) · `ExplainRegistryMiss` (شش خانواده) · `BuildExplanation` (پوستهٔ تضمین: هیچ‌وقت خط/عنوان خالی) |
| نوار ریسک برگشت | صاحب توضیح اختصاصی شد: مقادیر، فهرست شواهد امتیاز، و مسیر کنترل در فایل ریسک برگشت |
| انتخاب در هم‌پوشانی | قاعدهٔ قطعی: فاصله ← کوچک‌ترین ناحیه ← نام (قبلاً ترتیب تصادفی `ObjectsTotal`) |
| آزمون بدون موس | در پایان هر بازسازی روی همهٔ آبجکت‌های قابل‌کلیک: مرکز پیکسلی + همان hit-test زنده + همان سازندهٔ پنل |
| گزارش شاهد | `ICT_Assistant_Canonical_Click_Diag.csv` با تفکیک `RESOLVED` / `REGISTRY_MISS` (معتبر) / `NO_HANDLER` (نقص) |
| ابزار تازه | `tools/Validate-Phase44.ps1` — قفل استاتیک (کامل‌بودن، نبود پنل خالی، قطعیت) + بخش زمان‌اجرا روی همان CSV |

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `.ex5` در `…\khaleq\newICT\` (۲۰۲۶‑۰۹‑۲۲ ۰۰:۰۵:۱۱) |
| `Validate-Phase44` | **PASS=19 FAIL=0 PENDING=1** — بخش D8 تا reload واقعی PENDING می‌دهد (نه سبز دروغین) |
| `Verify-ModuleSplit` | OK — مرجع به `9E675F79…` (۱۲٬۱۶۶ خط) |
| baseline ماندگار | `05_TESTS_AND_VALIDATION/frozen/assembled_9E675F79.mq5` — سوئیچ `-Freeze` برای diff بایت‌به‌بایت در فازهای بعد |
| رگرسیون | **۲۳ ابزار، همه `exit 0`** — `Validate-Phase37` هم `exit 0` می‌دهد و ۶ قرمزی‌اش به یک `PENDING` (بند `B_STALE_REPORT`) تبدیل شد |
| دروازهٔ تازگی گزارش | `Validate-Phase37` زمان گزارش زمان‌اجرا را با **تازه‌ترین ورودی کامپایل** (پوسته + همهٔ `modules/*.mqh`) می‌سنجد، نه با مهر خود پوسته. اگر گزارش قدیمی‌تر باشد، کل بخش B `PENDING` می‌شود (نه PASS دروغین و نه شکست ناعادلانه) — الان: «گزارش ۲۰۲۶‑۰۹‑۲۱ ۰۹:۴۸:۵۵ < تازه‌ترین ورودی ۲۰۲۶‑۰۹‑۲۲ ۰۰:۰۴:۱۵ → یک reload»؛ `PASS=21 FAIL=0 PENDING=1` |


## فاز ۴۶ (۲۰۲۶‑۰۹‑۲۲) — درجهٔ سیگنال A+/A/B+/B/C در تیتر چارت

**شکایت:** «هنوز قابل اعتماد نیست، بخصوص اون تیتر بالا که قرار بود مشخص کنه سیگنال A+/A/B+/B».

**آمارهٔ مشکل:** (۱) اسکن کل واحد ترجمه → **صفر درجه‌بندی A+/A/B+/B**؛ تیتر هرگز ساخته نشده بود. (۲) امتیاز حرکتی روی نوار مرتب‌سازی نمی‌کرد: LOW ۲۹٫۲٪ · MEDIUM ۲۹٫۶٪ · HIGH ۲۹٫۳٪.

**کار انجام‌شده**

| قلم | جزئیات |
|---|---|
| تعریف تازه | مسابقه: +۲ ATR هم‌جهت بایاس در برابر −۱ ATR خلاف آن، داخل ۱۲ کندل؛ تساوی → حد ضرر |
| ابزار فیت | `tools/Fit-SignalGrade.ps1` — لیفت لگاریتمی-شانسی هر ویژگی از داده، آستانه‌ها از چارک‌های همان توزیع، سوئیچ `-Emit` برای شاهد یخزده |
| ماژول تازه | `modules/31_SignalGrade.mqh` — کد خانواده، سطل‌ها، جدول ضرایب، آستانه‌ها، نرخ برد هر درجه |
| نتیجهٔ سنجش | A+ ۵۲٫۲٪ (n=۹۲) · A ۴۱٫۲٪ (۳۰۶) · B+ ۳۵٫۵٪ (۷۰۷) · B ۳۱٫۲٪ (۱۱۱۵) · C ۲۵٫۸٪ (۳۲۹۹) — خط پایه ۲۹٫۴٪ |
| شاهد دروغ | نوشتن ردیف CSV از `UpdateReverseRisk` به `PersistReverseRiskEvidence()` منتقل شد و **بعد از** درجه اجرا می‌شود |
| نوار | `GRADE` سر ردیف، رنگ = درجه، و **عرض نوار از طول متن** (قبلاً ثابت ۶۴۰ و متن بریده می‌شد) |
| پنل | معنی درجه، معنی عدد برد و نمونه‌اش، معنی رنگ‌ها، و `why` (بزرگ‌ترین بالابرنده/کاهنده) |
| CSV | پنج ستون در **انتها**: `LevelFamily` · `Grade` · `GradeScore` · `GradeWinPct` · `GradeN` (خواننده‌های قدیمی دست‌نخورده) |

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `.ex5` در `…\khaleq\newICT\` (۲۰۲۶‑۰۹‑۲۲ ۰۴:۴۰:۳۴) |
| `Verify-ModuleSplit` | OK — `C0725861…` (۱۲٬۴۷۰ خط، ۳۱ ماژول)؛ diff بیرونی = **۱۲ هانک، همه فاز ۴۶** |
| `Validate-Phase46` | **PASS=21 FAIL=0 PENDING=0** — دروازهٔ زمان‌اجرا بسته شد |
| `Validate-PersianRender` | **PASS=50 FAIL=0** |
| رگرسیون | ۲۴ ابزار، **صفر شکست** — `Phase32`/`Phase35`/`Phase43` به سنجش حقیقت تازه به‌روز شدند و `Phase43` یک باگ دروغین (BOM → ستون غلط) داشت که رفع شد |

**محدودیت صادقانه:** نمونهٔ کالیبراسیون در `COMMON\Files` ترمینال است نه در مخزن؛ شاهد یخزده `05_TESTS_AND_VALIDATION/grade_coefficients.fixture.csv` است و ابزار قفل‌کننده ۲۶ ضریب + ۴ آستانه + ۵ نرخ را عددی با آن می‌سنجد. n برای A+ تنها ۹۲ است.

**دروازهٔ زمان‌اجرا (بسته شد):** در `04:53:48` نمونه‌ای روی `XAUUSD.x M15` بار شد و در بستهٔ کندل `01:15` اولین ردیف درجه‌دار نوشته شد — `famMITIG · grade=C · score=-0.110` — و ابزار قفل‌کننده همان سه مقدار را از ستون‌های ویژگی‌های همان ردیف بازمحاسبه کرد و **مو‌به‌مو یکسان** درآمد (`PASS=21 FAIL=0 PENDING=0`).

**اقدام کاربر:** برای هر چارت دیگری که می‌خواهد `GRADE` را ببیند، یک بار Remove → Refresh → Add (در MT5 هر چارت نمونهٔ مستقل است؛ چارت M1 تا `04:53:00` هنوز با بیلد قبلی می‌نوشت).

---

## فاز ۴۷ (۲۰۲۶‑۰۹‑۲۲) — سناریوی در انتظار (PENDING) و جدا شدن دفتر شاهد هر تایم‌فریم

**خواسته:** «ریپنت نمی‌خوام؛ ولی احتمال اینکه این اتفاق بیفته را بنویس، منتظر بسته شدن کندل بمون — و کمک کنه بدونیم می‌خواد چه اتفاقی بیفته. هر تایم‌فریم هم برای خودشه.»

**مسئله:** دروازهٔ برگشت پسرو بود؛ تا کندل HTF بسته نمی‌شد چیزی نمی‌گفت. در عوض، دفتر شاهد فقط نام **نماد** را داشت، پس دو چارت زندهٔ یک نماد، ردیف‌هایشان را در یک فایل می‌ریختند و هر اندازه‌گیری دو تایم‌فریم را قاطی می‌کرد.

| قلم | جزئیات |
|---|---|
| ابزار سنجش | `tools/Fit-PendingScenario.ps1` — اپیزودبندی بر اساس شناسهٔ سویینگ محافظت‌شده، وضعیت هر کندل بستهٔ HTF، فاصله با ATR جوین‌شده از دفتر ریسک |
| ماژول تازه | `modules/32_PendingScenario.mqh` — موتور + ردیف + هندلر توضیح فارسی |
| نتیجهٔ سنجش (XAUUSD.x · H4 · ۳۰۰ اپیزود) | <۰٫۲ ATR → **۳۹٫۱٪** (n=۲۳) · ۰٫۲‑۰٫۵ → ۰٪ (۳۹) · ۰٫۵‑۱ → ۰٪ (۸۱) · ۱‑۲ → ۱٫۸٪ (۱۶۸) · >۲ → ۰٪ (۲۴۵۶) · **خط پایه ۰٫۴٪** |
| قانون | سه ضمانت: فقط کندل بسته ورودی است · صفر نوشتن · یک‌بار در هر کندل بسته |
| ردیف | تک‌خط زیر نوار ریسک: وضعیت · ماشهٔ دقیق · فاصله · نرخ و نمونه · مقصد نقدینگی؛ کلیک = پنل آموزشی |
| جدا شدن تایم‌فریم | `ChartTfCode()` + نام فایل `…_Reversal_Diag_<TF>.csv` و `…_ReverseRisk_<symbol>_<TF>.csv` + ستون `ChartTF` در انتهای سرستون |

**شاهد زمان‌اجرا:** پنج دفتر جدا نوشته شد و ستون آخر هرکدام با نام فایلش یکی است — `H1 · M15 · M5 · M2 · M1` (همه `XAUUSD.x`).

| دروازه | نتیجه |
|---|---|
| Build | **0 errors, 0 warnings** — `…\khaleq\newICT\ICT_Assistant_Canonical_v0_1.ex5` (۲۰۲۶‑۰۹‑۲۲ ۰۵:۵۵:۵۱) |
| `Verify-ModuleSplit` | OK — `32A7FB3F…` ، ۱۲٬۸۸۲ خط، ۳۲ ماژول، diff بیرونی فقط هانک‌های فاز ۴۷ |
| `Validate-Phase47` | **PASS=21 FAIL=0 PENDING=0** |
| `Validate-Phase46` · `Validate-Phase12` | ۱۸/۰/۱ · ۷۷/۰/۰ (دو ابزار به شرط تازهٔ انتهای ستون‌ها به‌روز شدند) |
| رگرسیون | ۲۵ ابزار، صفر شکست |

---

**بازمانده (فاز ۴۷):** سبدِ نزدیک (۰٫۲ ATR) فقط ۲۳ نمونه دارد و روی یک نماد سنجیده شده؛ واحد سنجش «گذار مشاهده‌شدهٔ کندل بستهٔ HTF» است؛ و شاهد خط‌به‌خطِ `Click_Diag` برای ردیف تازه بعد از اولین بازسازی تاریخچه نوشته می‌شود.

---

## فاز ۴۸ (۲۰۲۶‑۰۹‑۲۲) — پالت رنگی چارت + یک‌خطی‌شدن «مسیر»

**خواسته:** «تداخل رنگ روی چارت را رفع کن (iFVG و True-Open روزانه و هفتگی QT سه رنگ جدا شوند) و دو ردیف درجه و سناریوی انتظار را در یک خط «مسیر» یکپارچه کن.»

**آمارهٔ مشکل (از اسکن خطوط رسم، نه از حافظه):** چند خانوادهٔ بی‌ربط دقیقاً یک رنگ داشتند — `clrMagenta` در اردربلاک نزولی · پنجرهٔ سیلور بولت · ناحیهٔ اولیهٔ روز · سطح طلایی OTE؛ `clrDodgerBlue` در اردربلاک صعودی · باکس سشن لندن؛ `clrGold` در خط ستاپ · POC · تعادل لگ؛ `clrOrangeRed` در DOL · نقدینگی HTF · سشن نیویورک · دروازهٔ تأییدشده؛ `clrTomato` در نقدینگی سقفی · خط روند بروکس · POC برهنه؛ `clrAqua` در OTE · پس‌زدگی صعودی · VAH/VAL؛ `clrDeepSkyBlue` در بریکر · POI برتر؛ و سه‌گانهٔ بنفش iFVG/QT‑روز/QT‑هفته.

| قلم | جزئیات |
|---|---|
| پالت | ۳۲ رنگ یکتا `PAL_*` در `02_Types.mqh` (پیش از همهٔ ماژول‌های رسم). انتخاب با **بیشینه‌سازی کمترین فاصلهٔ RGB** (حل عددی چیدمان نقطه در مکعب رنگ) |
| فاصله‌ها | کمترین فاصلهٔ کل جدول **۵۲٫۲** (قبلاً صفر بود) · سه خانوادهٔ مورد شکایت کاربر **۱۸۸ / ۲۵۹ / ۳۰۹** · نزدیک‌ترین فاصله به دو ورودی جهت **۵۹٫۱** |
| حالت در برابر خانواده | رنگ حالت (لمس‌شده · جاروشده · شکسته · باطل · بدون‌زنجیره) عمداً مشترک است — «یک معنی، یک رنگ» |
| هفت سهم اعلام‌شده | بستهٔ لندن=سشن لندن · عصر نیویورک=سشن نیویورک · POC برهنه=POC · EQ و OTE=سطح طلایی · میانهٔ رنج=حرکت سنجیده‌شده — ابزار قفل‌کننده فقط همین هفت را می‌پذیرد |
| باکس POI برتر | رنگ جهت گرفت (قبلاً `DeepSkyBlue` بود و با باکس بریکر قاطی می‌شد) |
| یک‌خطی‌شدن | `RenderRiskStrip()` حالا ردیف **مسیر** می‌سازد: درجه و بایاس ← سطح فعلی ← مقصد نقدینگی ← شرط برگشت و نرخش. قطعهٔ مسیر از `PendingFragmentFa()` (همان ماژول محاسبه) |
| پنل | یک کلیک، یک پنل، دو بخش: `ExplainRiskStripObject()` در پایان `ExplainPendingScenario(true)` را صدا می‌زند |
| ورودی‌های حذف‌شده | `InpShowPendingLine` · `InpPendingLineY` · `InpPendingLineWidth` — چیدمان همان `InpRiskStripY`/`InpRiskStripWidth` |
| پاک‌سازی | `CleanupLegacyPendingRow()` اشیای ردیف قدیمی را یک‌بار (با پرچم `g_pendRowCleaned`) حذف می‌کند |
| ضمانت عدم‌ریپنت | دست‌نخورده: تنها ورودی «آخرین کندل بسته»، صفر نوشتن، یک‌بار در هر کندل بسته |
...
**بازمانده:** (۱) کمترین فاصلهٔ پالت ۵۲٫۲ — سقف هندسی ۳۲ رنگ؛ ابزار قفل‌کننده اجازه نمی‌دهد رنگ تازه‌ای به دیگری بچسبد. (۲) رنگ‌های جهت دست کاربرند و ابزار مقدار **پیش‌فرض** را می‌سنجد. (۳) تأیید بصری چیدمان و فارسی نوار با یک `Remove → Refresh → Add` انجام می‌شود؛ من چارت را نمی‌بینم.

**حادثهٔ مستندسازی همین فاز:** نوشتن `مستندات فنی پروژه` با بازنویسی کل فایل انجام شد و بخش فازهای ۳۷ تا ۴۷ آن از دست رفت (پرونده بیرون از مخزن است و گیت نسخهٔ بازیابی نداشت). آخرین نسخهٔ کامیت‌شده (پایان فاز ۳۶) بازیابی شد و یک یادداشت هشدار + خلاصهٔ قاعده‌های فازهای ۳۷–۴۷ رویش افزوده شد. مرجع کامل همان رویدادها همین فایل است.
